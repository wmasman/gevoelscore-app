...
| `src/lib/api/` | **`@directus/sdk` v18** | Typed Directus client for reads + writes. Wrapped in domain-shaped Result functions in [`src/lib/auth/directus-auth.ts`](../../src/lib/auth/directus-auth.ts) (auth flow) and future `src/lib/api/` modules (data flow). Decision rationale in [docs/features/login/step-3-origin-check-and-directus-auth.md](../features/login/step-3-origin-check-and-directus-auth.md) and [frontend-setup.md](frontend-setup.md). |
