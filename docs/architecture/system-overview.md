...
```
                                   ...
                                   │  Region: ams, 512 MB, 1 machine     │
                                   │  ✅ DEPLOYED 2026-05-27 — image 65MB │
                                   │                                     │
                                   │  • /api/auth/{login,verify,logout}  │
                                   │  • /api/auth/2fa/{generate,enable}  │
                                   │  • middleware: gate every page      │
                                   ...
                                   └────────────────┬────────────────────┘
                                                    │
                                                    │ HTTPS via the Fly proxy
                                                    │ https://gevoelscore-backend.fly.dev
                                                    ▼
                                   ┌─────────────────────────────────────┐
                                   │  Directus 11.17.2                   │
                                   ...
                                   └────────────────┬────────────────────┘
                                                    ...
                                                    │ WireGuard mesh) — Postgres on
                                                    │ gevoelscore-pg.flycast:5432
                                                    ▼
                                   ┌─────────────────────────────────────┐
                                   │  Fly Postgres 18 (gevoelscore-pg)   │
                                   │  Region: ams, single node           │
                                   │  2 GB encrypted volume              │
                                   │  Always-on; daily volume snapshots  │
                                   │  (14-day retention)                 │
                                   └─────────────────────────────────────┘
```
...
5. **Directus SDK call** — `directus.items('day_entries').updateByQuery({ filter: { date: today } }, { score: 5 })`. Goes through the Fly proxy (`https://gevoelscore-backend.fly.dev`) — proxied traffic is what wakes a suspended backend machine; `.internal` DNS would bypass the proxy and hit a sleeping machine (see [ADR 0007](../decisions/0007-self-hosted-postgres-on-fly.md)).
...
7. **Fly Postgres** — applies the UPDATE. ~10–50ms typically. The DB itself is always-on; the first request after a *machine* suspend adds ~1.5–2.5s while the frontend/backend machines resume.
...
Total expected: well under 200ms warm; a cold wake after machine suspend measured ~1.4s (frontend) / ~2.6s (full login chain). Cardinal sub-10s budget has lots of room.
...
3. **Raw SQL** for one-off analysis via `fly proxy` + psql or `pg`-based scripts.
...
| **Directus → Postgres** | SQL over Fly private network (flycast) | Password in Fly secret; no public exposure — `gevoelscore-pg` has no public address |
...
| Fly Postgres | ✅ Live | Flat-cost single node; always-on. See [ADR 0007](../decisions/0007-self-hosted-postgres-on-fly.md). |
...
| Postgres views (`daily_observations`, `day_tags_flat`, `tag_correlations`) | ✅ Applied via psql (fly proxy) | Read-only query surface; see [queries-and-views.md](queries-and-views.md) |
...
| Next.js frontend code | ✅ Shipped | Next 15.5 + Tailwind v4 + Playwright. Login feature complete (8/8 steps). |
| Next.js Fly app deploy | ✅ Deployed (1 machine) | `gevoelscore-frontend.fly.dev` live. Smoke-tested end-to-end: middleware redirect, real Directus auth, cookie attrs all correct on HTTPS. |
...
- **Fly Postgres**: ~$2–3/mo (machine + volume)
...
| Backend up but writes failing | Check `fly status -a gevoelscore-pg` and `fly logs -a gevoelscore-pg` |
