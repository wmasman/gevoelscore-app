...
**Status**: **r2 LOCKED 2026-06-19** per [CONVENTIONS §2.2 + §2.3](../CONVENTIONS.md#22-methodology-md-before-locking-a-major-choice) audit hooks + [`hypothesis_lock_process.md §3.6`](hypothesis_lock_process.md) compression. Fresh-session audit ([`reviews/bout_level_recovery_dynamics-2026-06-19.md`](../reviews/bout_level_recovery_dynamics-2026-06-19.md), single report covering this MD + sub-MD) verdict PASS-with-caveats; §3.6 compression applied for r2 absorbs (mechanical; no architectural change; multiplicity-deferral user-confirmed pre-r2 2026-06-19). Co-locked with sub-MD [`bout_level_dose_response_calibration.md`](bout_level_dose_response_calibration.md) at the same commit.
...
| Match to Wiggers' verbatim claim | weak (HA-C4 v2 REJECTED demonstrates) | over-spec'd | partial (event presence + per-event summary captures threshold-crossings but discards within-event recovery-shape, which is what "stress doesn't decrease" names; per-event-feature variants converge toward bout-level by adding shape features) | strong (Wiggers names "stress doesn't decrease for a long time" — a *recovery time* claim) |
| Compositionality with existing primitives | high | low | medium | high (per-bout features compose with per-day aggregates) |
| n=1 single-subject suitability | corpus-rich but mechanism-blind | mechanism-flexible but reads in-sample | corpus-thin (sparse events × thin per-event summaries) | corpus-rich AND mechanism-shaped |

**Scope clarification on the event-only column** (r2 absorb 2026-06-19, audit L1 minor): "event-only analysis" in the table means *detect event onsets + report per-event count / per-event summary stats* (e.g. HA11's `u_dip_count` per day). Per-event-feature variants that add shape features (peak height, recovery slope) converge toward bout-level by construction — the bout is itself a per-event-feature richer than a count. The column is thin by deliberate scope: the audit-honest comparison is event-presence vs bout-feature-vector, not bout-level vs every-possible-per-event-feature-augmentation.
...
- **Firstbeat-input amplification at minute resolution** (r2 absorb 2026-06-19, audit L2 MEDIUM): per [`hrv_proxy_via_stress.md`](hrv_proxy_via_stress.md), the upstream Firstbeat-algorithm-derived stress signal is opaque (closed-source); the daily-aggregate level smoothed over minute-scale artefacts from the algorithm. Bout-level analysis at minute resolution SURFACES artefacts that daily-aggregate hid. Specifically: peak-detection rules treating Firstbeat output as a clean physiological signal inherit any minute-scale algorithmic noise + non-linear sensor-to-stress mappings (HR + respiration + motion-residual contributions the daily mean averages over). A bout's `peak_height = 80` may reflect HR + respiration spike rather than HRV change. Daily aggregate smooths this; bout-level features amplify it. This is a known methodology limitation, not a bug. Sub-MD [`bout_level_dose_response_calibration.md`](bout_level_dose_response_calibration.md) §7 caveat 4 carries the equivalent caveat for the recalibration. The framework-validity check on HA11 v1 (§6) is the primary mitigation: if algorithmic noise dominates, the bout-level reproduction of HA11 v1's SUPPORTED-on-train signal will fail.
- **Pacing-behaviour mask risk** (r2 absorb 2026-06-19, audit L1 minor): per PM brief §7 ([`research-pm-brief-bout-level-recovery-pivot-2026-06-19.md`](file:///C:/Users/Gebruiker/.claude/plans/research-pm-brief-bout-level-recovery-pivot-2026-06-19.md)), if the train-era flatness in HA-C4 v2 (REJECTED 2026-06-18) is driven by the participant's pacing behaviour rather than by daily-aggregate's resolution limits, bout-level analysis won't recover the signal in train either. §5.5 + §6 below carry the substantive treatment of this risk; the framework-validity check (§6) restricted to unmedicated × train × calm-day pool is the primary mitigation — it tests whether bout-extraction is well-defined independent of the pacing-mask question.
...
**Primary-operand handling of `transient_flag = True` bouts** (r2 absorb 2026-06-19, audit L3 minor): pinned — **transient bouts are INCLUDED in the primary operand** (`bout_n_fast_recovery_day` + all per-day aggregations in §4) without down-weighting. Rationale: (a) cross-test consistency with HA11 v1's framework-validity reference frame, which does not down-weight any event class; (b) determinism — the primary operand must be a single, unambiguous count, and weighted-inclusion introduces a secondary parameter (the down-weight) that this MD declines to pre-commit. The "reported but down-weighted" framing in the paragraph above is RETAINED for descriptive companion reads + sensitivity arms; downstream HA pre-regs may exclude transient bouts in their own sensitivity arms (analogous to motion-clean-only per §3.4) but cannot relax the primary's transient-inclusion default. Verification of the framework-validity gate (§6) inherits this default.
...
**No-intensity-record handling** (r2 absorb 2026-06-19, audit L2 minor): when the per-minute intensity record is absent across the bout window (the device did not log an intensity tier at any minute in `[t_onset, t_end]`), `motion_confound_flag = NaN` (NOT `False`). Downstream filtering MUST treat NaN as a coverage-gap, not as a no-confound signal — encoding absent-record as False would silently fold device-coverage gaps into the motion-clean primary operand. (This is a deliberate departure from [`stress_low_motion_primitive.md` §3.2](stress_low_motion_primitive.md)'s daily-aggregate convention, where absent-record defaults to low-motion because ~81% of HR-sampled minutes have no record at the day level per §3.3a; at bout level — a much shorter window — the same defaulting would conflate true motion-clean bouts with coverage-gap bouts. Cross-primitive consistency at the *per-day* aggregation column `bout_n_motion_clean_day` is preserved by re-applying the primitive's convention at aggregation time; the per-bout flag stays NaN-preserving.)
...
**Empirical anchor for the long-memory expectation** (r2 absorb 2026-06-19, audit L4 minor): per [`recovery_arc/findings.md`](../analyses/descriptive/trajectory/recovery_arc/findings.md) (commit `24dad02`, 2026-06-19), **6 of 7 Stratum-4 cells fire the factor-of-2 E[L]\* flag** on the HA-P6 channel set — the empirical long-memory anchor that bout-level analyses inherit at within-day resolution (within-day autocorrelation likely tracks the same physiological persistence the per-day E[L]\* surfaces). The sub-MD [`bout_level_dose_response_calibration.md`](bout_level_dose_response_calibration.md)'s per-bout β recalibration honestly inherits this: bout-level β-estimate CIs are expected to widen under the long-memory structure relative to a naive iid bootstrap. The framework-validity gate (§6) operates at day-level on `bout_n_fast_recovery_day` — the right unit because E[L]=7 directly applies; per-bout β recalibration (sub-MD) runs at the within-day mixed-effects level where day-cluster SE is the analogue.
...
**Multiplicity-correction strategy — deferred to per-HA-pre-reg (PM-confirmed 2026-06-19)**: per user-affirmation 2026-06-19 (in response to the audit Layer 4 fire on the PM brief §7 advisory recommendation that this MD pre-commit a methodology-level multiplicity rule), the methodology MD intentionally defers multiplicity-correction *choice* to each downstream HA pre-reg. Per HA design (e.g. bout-level features × per-day aggregations × cross-phase arms × sensitivity sweeps) the appropriate correction (Holm step-down / Westfall-Young permutation / Bonferroni / none) varies; pre-committing one strategy at the methodology level would over-constrain HA design. Each HA pre-reg using this MD's operand states + justifies its multiplicity-correction choice in its §5 inferential framework. The single-cell-headline + Holm-step-down companion default recommended above stands as the project-canonical *starting* pattern; departures require explicit justification per `hypothesis_lock_process.md §3.2`. This confirmation closes the audit's L4 fire by explicit user-confirmation of the deferral (auditor-flagged as defensible-but-needs-confirmation).
...
**Relationship to HA11 v1's locked three-criterion set** (r2 absorb 2026-06-19, audit L3 minor cross-cite): HA11 v1's pre-registered SUPPORTED bar (per [`HA11/hypothesis.md` §5](../analyses/hypotheses/HA11-stress-udip/hypothesis.md#5-pre-registered-falsification-criterion)) is the three-criterion shape (a) frequency ≥ 60%, (b) discrimination ≥ +15pp, (c) median signed-z ≥ N_std/2. The framework-validity gate's three bars map as follows:
- **Bar 1 (directional sign)** is implicit in HA11 v1's (b) — discrimination only counts in the positive direction.
- **Bar 2 (≥ +12.8pp)** is a *comparability band* derived from HA11 v1's *observed* train discrimination (+22.8pp, see [`HA11/result.md`](../analyses/hypotheses/HA11-stress-udip/result.md), ±10pp), NOT from its *locked* SUPPORTED bar (≥ +15pp). This is intentional: framework-validity asks whether bout-level reproduces the SIGNAL HA11 v1 observed on-train, not whether it re-clears HA11 v1's pre-registered threshold. A bout-level result of +14pp would clear HA11's locked bar but fail the comparability band; that disagreement is the framework-validity-relevant fact.
- **Bar 3 (p < 0.05)** is *additional to* HA11 v1's criterion set — HA11 v1 uses non-parametric thresholds (a)-(c), not p-values. The p-value bar here is the methodology-MD's own discipline (block-permutation null at E[L]=7 per §5.1); it constrains the framework-validity gate to require both comparability-of-magnitude AND statistical-significance-against-the-block-null. HA11 v1's (c) magnitude criterion is NOT mapped — the framework-validity gate's three bars do not encompass HA11 v1's (c). A bout-level result that clears bars 1+2+3 here but fails HA11 v1's (c) would be reported as a partial-framework-validity finding; the methodology MD's r2 (or v2) would have to decide whether HA11 v1's (c) is load-bearing for framework-validity (current discipline: it is not — bars 1+2+3 suffice).
...
**Per-bout-n reporting discipline at framework-validity result-time** (r2 absorb 2026-06-19, audit L4 minor — [CONVENTIONS §3.6](../CONVENTIONS.md#36-named-counts-discipline) named-counts hook): the HA pre-reg implementing the framework-validity gate (HA11-bout-redo) MUST report per-arm `n_bouts` AND per-cell `n_bouts × n_days × n_bouts_per_day` at result-time. Per-bout n is a derived quantity (depends on per-minute stress trace coverage + bout-extraction rule); under-reporting would obscure the framework-validity gate's effective power. This obligation extends to all downstream HA pre-regs using this MD's operand: report per-bout-n per arm + per-cell-n explicitly in the result.md per-cell table, anticipating that bout-counts vary across arms in ways the day-counts do not.
