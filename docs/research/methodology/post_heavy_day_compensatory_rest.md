...
**Status**: **LOCKED r1 2026-07-15** post-review absorption. Fresh-session `/research-methodology-review` verdict **DEFENSIBLE with revision** at [`reviews/methodology-post_heavy_day_compensatory_rest-2026-07-15.md`](../reviews/methodology-post_heavy_day_compensatory_rest-2026-07-15.md); 4 substantive absorbs applied pre-lock (see §13 lock log row 3). Producer-mode artefact per [CONVENTIONS §1.2](../CONVENTIONS.md#12-reviewer-mode-claude-reads--writes-a-reasoned-report--recommendations); Stage D descriptive audit runs against this operand.
...
- **Inclusive**: all episode-ends (n=314 gap=0), with the trajectory interpreted as "response to the entire multi-day heavy-load context spanning the window", not to the target episode alone. Under inclusive policy the block-permutation null (§7.10 below) uses block length ≥ w to preserve the autocorrelation structure the policy admits.
...
| `stress_mean_sleep` | Garmin sleep-window stress (HRV-proxy) | 97.0% | Autonomic overnight proxy. [`sleep_metrics.md`](sleep_metrics.md) Family D (physiological during sleep). |
| `all_day_stress_avg` | Daytime Garmin stress mean | 98.6% | Autonomic waking proxy. Project-level operand, NOT in [`sleep_metrics.md`](sleep_metrics.md) taxonomy (non-sleep-scoped); catalogued in [`DATA_DICTIONARY.md`](../DATA_DICTIONARY.md) §7B. Firstbeat-opacity caveat inherited per [`hrv_proxy_via_stress.md`](hrv_proxy_via_stress.md). |
| `bb_lowest` | Body-battery daily floor | 98.6% | Autonomic reserve proxy. Project-level operand, NOT in [`sleep_metrics.md`](sleep_metrics.md) taxonomy (day-scoped, not sleep-window-scoped); catalogued in [`DATA_DICTIONARY.md`](../DATA_DICTIONARY.md) §7B. Firstbeat-opacity caveat inherited per [`hrv_proxy_via_stress.md`](hrv_proxy_via_stress.md). |
| `bb_overnight_gain` | Overnight body-battery gain | **38.9%** | **Sensitivity-only** — see §6.2.1 below. [`sleep_metrics.md`](sleep_metrics.md) Family E (BB overnight recovery). |
| `hr_median_waking` | Median heart rate over the FIT-derived waking window (from `sleep_end_gmt` to next `sleep_start_gmt`); excludes sleep-window minutes; drops off-wrist samples via FIT-native filter | 98.4% | Waking-HR proxy — see §6.2.2 caveat. Computation-path defined in [`docs/research/pipeline/01_extract/`](../pipeline/01_extract/) FIT-monitoring extraction; NOT Garmin's proprietary "RHR" column. |
...
Vector of per-d+k means for the heavy-arm and matched-ordinary-arm, with bootstrap 95% CIs (block bootstrap per §7.10). Primary descriptive output; feeds the trajectory-plot deliverable at Stage D.
...
### 7.10 Null-hypothesis inference machinery (bootstrap + permutation + multiplicity)

**Bootstrap CI construction**: bootstrap resamples are drawn **at the episode-end level** (per §3.1 unit-of-analysis lock). Episode-ends across the corpus are sufficiently temporally separated (§5 audit: median 1d span for gap=0 episodes, distributed across ~4 years) that per-episode resampling with **block length = 1** is defensible — episode-end trajectories are treated as approximately independent draws. **B = 10,000 resamples** per outcome × window × overlap × stratum combination. Reported 95% CI = percentile [2.5, 97.5]. Convention inherited from parent MD [`bout_level_recovery_dynamics.md`](bout_level_recovery_dynamics.md) §5.3.

**Block-permutation null** (for arm-label significance testing, if any Stage H pre-registration invokes it): heavy-vs-matched-ordinary labels are permuted while preserving the temporal blocking structure. Under the **strict overlap policy**, block length = 1 (episode-ends independent). Under the **inclusive overlap policy** (§5.2), block length ≥ w for the window under analysis, preserving the within-window autocorrelation the policy admits. **B = 10,000 permutations**. Two-sided p-value = fraction of permutations with |Δ_permuted| ≥ |Δ_observed|. Convention consistent with [`methodology/permutation_null_block_length.md`](permutation_null_block_length.md).

**Multiplicity policy** (pre-committed 2026-07-15 as pre-lock revision per methodology review): Stage D reports all combinations descriptively with per-arm bootstrap CIs; no inferential multiplicity correction at Stage D (descriptive-before-inference per [CONVENTIONS §2.1](../CONVENTIONS.md#21-descriptive-before-inference)). If Stage H pre-registration follows (per §8 branch verdicts), the pre-reg specifies its own multiplicity correction family — the project-canonical default is **single-cell headline lock + Holm step-down across companion cells** per parent MD [`bout_level_recovery_dynamics.md`](bout_level_recovery_dynamics.md) §5.2 (also inherited by HA-C4c / HA-C4cp Holm-at-7-cells discipline). Companion set for a Q24 Stage H pre-reg would be: 4 sensitivity arms (§10 caveats) + intensity-stratified reads (§9). Cross-outcome multiplicity across the 5 autonomic channels × 3 windows is NOT corrected at Stage D — per-channel per-window branch verdicts (§8) are the reporting unit, and channel-disagreement is a substantive finding, not a multiplicity correction target.

### 7.11 Trajectory-detrend sensitivity per CONVENTIONS §3.7 (envelope-drift confound)

**Purpose** (pre-committed 2026-07-15 as pre-lock revision per methodology review): the corpus's LC recovery trajectory is uneven across the analysis window (crash rate ~10/year 2023-2024 → ~2/year 2025-2026 per [`analyses/hypotheses/registry.md`](../analyses/hypotheses/registry.md); heavy-day rate 47.4% in 2026 vs ~35% 2023-2025 per Stage -1 audit §2). Raw autonomic-channel trajectory contrasts may reflect envelope drift rather than event-triggered response. **Two sister-MD findings from `intervention_effects_descriptive.md` reversed under detrending** (raw p<.001 → detrend p=0.62 on `stress_mean_sleep` × 2022-09-22; raw p<.001 → detrend p=0.96 on `bb_overnight_gain` × 2026-03-20) — a direct precedent for the same reversal risk on Q24 raw trajectory findings.

**Method** — mirrors the sister MD's `linear_detrend_on_pre` helper adapted to trajectory contrasts:

- For each episode-end at day D and each outcome operand o:
  - Fit linear trend `y = a + b·t` (t = days-from-D) on the outcome column `o` over the pre-window `[D-30, D-1]` (30 days pre-episode, excluding the episode day itself). Pre-window size = 30d is inherited from parent MD reference-window pattern per [`bout_level_recovery_dynamics.md`](bout_level_recovery_dynamics.md) §3.2.2; a smaller-pre-window sensitivity (e.g. 14d) is reported at Stage D.
  - Extrapolate the fitted trend forward through the post-window `[D+1, D+w]`.
  - Subtract the extrapolated baseline from both the heavy-arm and the matched-ordinary-arm observations at each d+k.
  - Recompute all §7.1-§7.9 trajectory statistics on the detrended values.

- **Matched-ordinary comparator arm** uses the same-day detrend: for a matched-ordinary day D_ord, fit trend on `[D_ord - 30, D_ord - 1]` and extrapolate through `[D_ord + 1, D_ord + w]`. Comparator arm detrending uses each comparator day's own pre-window, preserving the paired-detrend semantics.

- **Insufficient pre-window handling**: if the outcome operand has <15 valid data points in the pre-window (i.e., 50% coverage over 30 days), the episode's detrend is flagged NaN and the episode drops from the detrended arm for that outcome. Report per-outcome per-window sample size for both raw and detrended arms.

**Reporting**: report **raw and detrended trajectory summary statistics side-by-side** at Stage D for every autonomic outcome (§6.2) + subjective outcome (`gevoelscore`). Sleep-quantity + sleep-architecture outcomes (§6.2 Family A) are less-affected by envelope drift (Miller 2022 timing/duration κ=0.35 vs staging κ=0.25) but the detrend is still computed and reported for completeness.

**Escalation-to-caveat rule**: if any (outcome, window) raw trajectory shows signal (per §8 decay-screen firing) but the detrended trajectory does not, flag the outcome as **trajectory-confound-suspect** in the Stage D report and add to §10 caveat list as a finding-specific caveat. The detrend-fragility itself is a caveat-class item per [CONVENTIONS §4.2](../CONVENTIONS.md#42-caveats-yes-a-priori-claims-no) — it does not overturn the raw verdict on its own, but it is load-bearing for downstream interpretation. Any Stage H pre-registration on a raw-trajectory finding that does not survive detrending must justify why the raw reading carries more weight than the detrended reading.

**Anchor** (from methodology review 2026-07-15): the sister `intervention_effects_descriptive.md` §3/§6 defines `linear_detrend_on_pre` at pre-vs-post-median level; this MD adapts the pattern to per-day trajectory contrasts. Both mechanisms remove envelope drift as a confound; the trajectory adaptation preserves per-day resolution rather than collapsing to a scalar median-diff.
...
8. **Envelope-drift asymmetry in the comparator pool (L2.2 stationarity)** (pre-committed 2026-07-15 as pre-lock revision per methodology review): the matched-ordinary comparator pool (§4.1) draws from the full LC-era corpus 2022-04-04 → 2026-06-05. The corpus's underlying envelope (crash rate, exertion levels, subjective baseline) drifts materially across this span (crash rate ~10/year 2023-2024 → ~2/year 2025-2026); a matched-ordinary day in 2023 is drawn from a structurally different physiological + behavioural environment than a matched-ordinary day in 2026. **Under strict-clean overlap policy (§5.2)** the small comparator pool per outcome may end up era-imbalanced relative to the trigger-side heavy-episode-end distribution, distorting the contrast magnitude even if trajectory shape is preserved. **Stage D handling**: (a) the trajectory-detrend sensitivity (§7.11) partially mitigates by removing per-episode linear pre-trend; (b) an additional era-stratified comparator sensitivity arm — restrict matched-ordinary comparator to same-era days as the heavy-episode (e.g. calendar-year match) — is reported as a §7.11 companion when a primary finding fires. If the era-stratified read shows substantially attenuated contrast, escalate the envelope-drift confound from caveat-class to finding-specific caveat per [CONVENTIONS §4.2](../CONVENTIONS.md#42-caveats-yes-a-priori-claims-no). Caveat-class in the pre-committed structure; not corrected in the primary read.
...
| r1 review absorption pre-lock (LOCK) | 2026-07-15 | Fresh-session `/research-methodology-review` verdict **DEFENSIBLE with revision** at [`reviews/methodology-post_heavy_day_compensatory_rest-2026-07-15.md`](../reviews/methodology-post_heavy_day_compensatory_rest-2026-07-15.md). Four substantive absorbs applied pre-lock (not a post-lock r2 revision): (1) **NEW §7.11 Trajectory-detrend sensitivity per CONVENTIONS §3.7** — highest-priority review fire (L3.2 + L4.7); inherits `linear_detrend_on_pre` pattern from sister [`intervention_effects_descriptive.md`](intervention_effects_descriptive.md) §3 + §6; per-episode 30d pre-window linear extrapolation; detrended trajectory summary statistics reported side-by-side with raw for every autonomic + subjective outcome at Stage D; escalation-to-caveat rule on divergence. Direct precedent: two sister-MD findings reversed under detrending (`stress_mean_sleep` × 2022-09-22 raw p<.001 → detrend p=0.62; `bb_overnight_gain` × 2026-03-20 raw p<.001 → detrend p=0.96). (2) **NEW §7.10 Null-hypothesis inference machinery** — L3.1 + L3.3 substantive fires; fixes ambiguous "§7.9" refs (§7.9 is first-crossing-day, not bootstrap); pre-commits block-bootstrap block length = 1 for strict-clean per-episode resampling + block length ≥ w for inclusive-policy block-permutation; B = 10,000 for both; pre-commits multiplicity policy (descriptive-only at Stage D + parent MD's canonical single-cell headline lock + Holm step-down at Stage H). (3) **NEW §10 caveat 8 Envelope-drift asymmetry (L2.2 stationarity)** — substantive Layer-2 fire; matched-ordinary comparator draws from full LC-era (2022→2026 crash rate 10/year→2/year drift); §7.11 detrend partial mitigation + era-stratified comparator sensitivity arm as §7.11 companion. (4) **§6.2 taxonomy reconciliation** — L4.8 partial fire; explicit column-Notes distinguish [`sleep_metrics.md`](sleep_metrics.md) Family D operands (`stress_mean_sleep`) from project-level operands (`all_day_stress_avg`, `bb_lowest`) that are day-scoped or waking-scoped and catalogued in DATA_DICTIONARY, not sleep_metrics.md; `hr_median_waking` computation-path pinned (FIT-derived waking window from `sleep_end_gmt` to next `sleep_start_gmt`, excludes off-wrist samples, NOT Garmin's proprietary "RHR" column). **STATUS transitions**: DRAFT r1 → **LOCKED r1**. |
