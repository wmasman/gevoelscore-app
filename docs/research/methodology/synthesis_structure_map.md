...
**Status**: **LOCKED r3 by user acceptance 2026-06-24**. r2 was LOCKED 2026-06-23 (Option γ canonical 4-stage closure per the Wave-5.5 pattern: r1 drafted → fresh-session methodology review REVISION RECOMMENDED → r2 substantive absorb of all 4 required + 8 recommended actions → user acceptance per review's explicit "can plausibly LOCK after absorbing the items in §7 below, without a second fresh-session review pass" allowance per the §3.6-compression analogue for non-blocking r1 fires). r3 propagates the upstream plan r4 → r5 absorption (addition of §3.12 subject-narrative commentary layer) into this map: §5 preface paragraph added, K-stress-fatigue-monitoring and K-bout-recovery-signal cells marked commentary-eligible. Authored 2026-06-23 by Claude under user interview, per §11 step 5 of [`_plan_results_analysis_layer.md`](_plan_results_analysis_layer.md) (LOCKED r5 2026-06-24). r1 → r2 absorbed a fresh-session `/research-methodology-review` (verdict REVISION RECOMMENDED, report at [`reviews/methodology-synthesis_structure_map-2026-06-23.md`](../reviews/methodology-synthesis_structure_map-2026-06-23.md)) that caught one substantive structural issue (C-bout-recovery collapses framework-vs-substantive distinction) and three discipline gaps (§3 coherence-reads cross structure-vs-synthesis line; L-ID citation obligation only surfaced in cross-refs; §7 lock-log wording risked conflating user sign-off with peer-review). All four required + all eight recommended actions absorbed in r2. r3 is mechanical propagation only — no methodology-review re-pass required since the substantive change is upstream in plan r5 (which absorbed without re-review per user pattern). See §7 lock log for per-revision diff.
...
**§3.12 subject-narrative commentary eligibility** (added at plan
r5, 2026-06-24): every tier-1 or tier-2 active construct is
commentary-eligible — meaning the Stage A `construct-*.md` artefact
may carry an optional subject-narrative commentary section
attached to the formal tier claim, with the §3.12 discipline rules
(subject-attribution, language-bounded, cannot-promote-tier, cannot-
cite-downstream, patient-audience-translation-only). Per-row
eligibility marked in the table.
...
|---|---|---|---|---|---|---|---|
| K-stress-fatigue-monitoring | Stress-fatigue daily monitoring signal | T-stress-fatigue-pacing | **Monitoring (tier-1, §3.10-PPV-exempt)** — descriptive shape only; not a weakened tier-2 claim. Informative-pattern (tier-2) blocked by single-cluster evidence + direction-anomaly with Wiggers' canonical claim. **§3.12 commentary-eligible**: tier-1 formal claim can carry optional subject-narrative commentary in the Stage A `construct-*.md` artefact, rendered into patient-audience translation track only. | Predictive-use tier **blocked** at this stage: no forward-validation HA exists; HA-C3 v2 REJECTED + HA-C3p PARTIAL together cap claims at descriptive monitoring (tier-1). A forward-validation HA testing "tomorrow's gevoelscore predictable from today's stress bin" would be needed for tier 2+. Per locked-plan §3.10, tier-1 monitoring claims do not require PPV-with-base-rate. §3.12 commentary cannot promote tier — only forward-validation HA can. | L1 applies, L2 applies (unmed only), L3 applies (Garmin signal), L4 applies (Wiggers prior), L5 NA (no v24 primary), L6 applies (gevoelscore), L7 applies (gating dropouts) | ACCEPTED | 2026-06-23 |
| K-bout-recovery-signal | Bout-level recovery signal (within-day) | T-within-day-recovery | **Informative-pattern (tier-2, §3.10-PPV-required)** — effect-size demonstrated but small. Predictive-use (tier-3) blocked by §3.10 hard predictive gate. **§3.12 commentary-eligible**: tier-2 formal claim can carry optional subject-narrative commentary in the Stage A `construct-*.md` artefact, rendered into patient-audience translation track only. | Predictive-use tier **blocked**: effect sizes (Cliff's δ = +0.120; +20.26 pp discrimination) are modest; bar-failures on both HAs (HA11-bout-redo p-value, HA-C4c effect-size) leave open whether the operand is at corpus detection limits. **Tier-2 requires PPV-with-base-rate per §3.10** — to be computed against crash base rate (~2/year residual per RESEARCH-REPORT §5.2 precedent) at Stage A time. §3.12 commentary cannot promote tier — only forward-validation HA can. | L1 applies, L2 applies (cross-phase pooled per HA-C4c), L3 applies (bout-derived Garmin), L4 applies (Wiggers prior), L5 NA (no v24), L6 NA (gevoelscore not in primary cell), L7 applies (gating + cross-phase coverage) | ACCEPTED | 2026-06-23 |
...
| 2026-06-24 | Revised r2 → r3 (mechanical propagation of upstream plan r5 §3.12) | Plan r4 → r5 absorbed a user-requested mid-implementation refinement adding §3.12 subject-narrative commentary layer (sibling to §3.10 hard predictive gate and §3.11 follow-up suggestions). This map's r3 propagates the §3.12 addition downstream: §5 preface paragraph added on commentary eligibility; K-stress-fatigue-monitoring (tier-1) and K-bout-recovery-signal (tier-2) cells marked **§3.12 commentary-eligible** with the discipline rules cited inline (cannot promote tier; patient-audience translation track only; hard separations preserved). No structural change to clusters / topics / reserved slots; no methodology-review re-pass since the substantive change is upstream in plan r5 (which absorbed without re-review per user Option-γ pattern). |
| 2026-06-24 | **LOCKED r3** | **User acceptance** (recommended option: "Bump map to r3 with a lock-log entry for the §3.12 propagation"). Status of all r2-LOCKED rows remains LOCKED; the §3.12 commentary-eligible notes are now LOCKED additions to K-stress-fatigue-monitoring and K-bout-recovery-signal cells. Implementation proceeds to §11 step 6 (draft six guides). |

**r3 LOCKED 2026-06-24**: see entry above for the LOCK closure.
