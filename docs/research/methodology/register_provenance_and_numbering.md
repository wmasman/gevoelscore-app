...
**Status: LOCKED r3 — 2026-07-09 by user acceptance.** Drafted 2026-07-08; r2 + r3 closed two fresh-session `/research-methodology-review` passes ([r1 review](../reviews/methodology-register_provenance_and_numbering-2026-07-08.md), [r2 review](../reviews/methodology-register_provenance_and_numbering-2026-07-09-v2.md)); locked at r3 per the r2 review's clear-to-lock-after-L2.4-fix verdict. Phase-1/2/3 application work is now unblocked. See §11 for the revision + lock log.
...
|---|---|
| `idea` | register-only; no operationalisation yet |
...
The reconciled Beyond roster, keyed to the live `/beyond` slugs, showing the crosswalk and the hybrid dual-attribution:
...
- **`what-the-watch-catches`** — **P8** (autonomic fingerprints of load; delivered via the R4 card; `descriptive / done`). Pure Beyond (note-tagged loads, not a Wiggers claim).
- **`changing-crash`** / **`character-flip`** — the observation (**P9**, `descriptive / done`) and its scoped test (P9's tested sibling, `tested / scoped`). One thread, two site items — the other many-to-one case.
- **`emotional-trigger`** — **P10** (emotional load as a crash *trigger*; `tested / parked`, held for a pre-registered test).
...
- **`best-in-the-middle`** (activity-conditioning on the C3 stress→fatigue curve, R21 / site `C3 × E`) — **membership call pending, not pre-decided here.** It sits on the §2.1 boundary: an activity-*confound test* Wiggers does not make (argues distinct-claim → Beyond) built directly on her C3 curve + C4 coupling (argues operational-refinement → Guide-extension). It has no standing routing and no executed test yet (scoped), so it is a fresh Phase-1 call. Default by analogy to C4b: treat as a `guide-extension` surfaced on `/beyond` unless the user judges the confound-test a distinct-enough theoretical claim to mint a P#.

C4b and the activity-map are the two items R36's starter crosswalk filed under "beyond" while its stated rule said "not Wiggers-derived." §2.1 resolves C4b as a Guide operational-refinement editorially surfaced on `/beyond` (**not** a Beyond mint — this corrects an r1 draft that reversed the 2026-06-14 routing), and flags the activity-map as a pending Phase-1 membership call.

**The `P8`–`P10` ids above are illustrative pending Phase-1 minting** (Rule 1); Phase 1 assigns the actual integers when the entries are drafted.
