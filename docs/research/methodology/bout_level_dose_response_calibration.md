...
**Verdict-rule precedence** (r4 absorb 2026-06-23, audit L4.8/PE.4 precision): the §3.4 NULL pre-spec is checked first. Only if the NULL pre-spec fails (i.e., the four-condition gate is not met) does the rule fall through to CONFIRMED / weakly_consistent / partial / REJECTED. This precedence pins the implementation choice: when the dose-β is statistically indistinguishable from zero, "sign of the noise" is the wrong lens for verdict assignment; NULL is the right verdict. The afbouw-window sign-discordant signal on a feature whose buildup-headline cell is NULL is logged as a substantive observation in result.md and §6 inheritance-row notes, not promoted to REJECTED at the buildup-headline.
...
| `recovery_half_life` | weakly_consistent | +1.096 [−0.217, +2.410] p=0.102 | +0.957 [−0.350, +2.264] p=0.151 | +0.014/day [n.s.] | **Sensitivity-only** (downstream HA pre-regs treat Approach A as sensitivity arm, not primary) |
| `decay_slope` | NULL | +0.019 [−0.060, +0.098] p=0.644 | +0.013 [−0.029, +0.054] p=0.535 | n.s. flat | **Dose-naive freely usable** (NOT load-bearing) |
| `AUC_above_baseline` | weakly_consistent | +52.4 [−49.5, +154.3] p=0.314 | **−20.5 [−112.2, +71.2] p=0.661** (sign-discordant in afbouw; buildup-headline weakly_consistent stands) | +1.66/day [n.s.] | **Sensitivity-only** (buildup-headline only; afbouw sign-discordance is a cross-window-coherence caveat for any pre-reg invoking the dose-adjusted variant in afbouw) |
| `bout_count_day` (≡ pipeline `bout_n_per_day`) | NULL (headline); afbouw shows sign-discordant signal | −0.020 [−0.121, +0.081] p=0.698 | **−0.102 [−0.202, −0.002] p=0.045** (wrong direction vs +1 prior; sign-discordant across all 7 sens specs; p<0.05 on 5 of 7 — primary + sens_B + sens_D + sens_E + sens_F; sens_A bootstrap p=0.068 + sens_C lagged_lcera p=0.084) | +0.005/day [n.s.] | **Dose-naive freely usable** at buildup-headline; downstream HA pre-regs touching `bout_n_per_day` in **afbouw** must pre-spec a sensitivity arm covarying for `dose_plasma_mg` to disambiguate the wrong-direction signal |
...
**Status**: **r4 LOCKED 2026-06-23 by user acceptance** + §3.6 compression of the fresh-session audit's PASS-with-caveats verdict (audit at [`reviews/bout_level_dose_response_calibration-r3-2026-06-22.md`](../reviews/bout_level_dose_response_calibration-r3-2026-06-22.md); explicit compression-eligibility note in §5 of the audit). r4 absorbs 6 mechanical wording / numerics reconciliations against the CSV source-of-truth; no architectural change, no verdict-assignment change, no STOCKTAKE §6 framing change. See §8 revision log r4 entry for the full closure list. r2 spec (§3-§5) remains LOCKED unchanged; r3 (§6 population) status preserved as historical anchor; r4 layered on top as the mechanical-compression LOCK signal.
...
| r4 | 2026-06-23 | **§3.6-compression absorb of fresh-session audit fires** ([`reviews/bout_level_dose_response_calibration-r3-2026-06-22.md`](../reviews/bout_level_dose_response_calibration-r3-2026-06-22.md), verdict PASS-with-caveats). Six mechanical absorbs, none architectural, none changing any §6 verdict assignment or the STOCKTAKE §6 architectural-implications framing: (1) **§6 numerics reconciliation against CSV source-of-truth** — `recovery_half_life` afbouw β corrected from `+0.564 [−0.213, +1.342] p=0.155` to **`+0.957 [−0.350, +2.264] p=0.151`** (verdict weakly_consistent stands); `AUC_above_baseline` afbouw β corrected from `+27.7 [−97.6, +153.1] p=0.661` sign-concordant to **`−20.5 [−112.2, +71.2] p=0.661` SIGN-DISCORDANT** (buildup-headline weakly_consistent stands; cross-window-coherence caveat added to inheritance note); same numerics fixed at three surfaces (§6 table + result.md §3 + result.md §4 dynamics-vs-level read paragraph) per audit L1.2; (2) **§6 `bout_n_per_day` afbouw sign-discordance count precision** — corrected from "robust across 6 of 7 sens specs" to "sign-discordant across all 7 sens specs; p<0.05 on 5 of 7" per audit L4.6 named-counts discipline; same precision applied at STOCKTAKE §6; (3) **§3.5 verdict-rule precedence note** — explicit precedence (§3.4 NULL pre-spec checked first; only if NULL pre-spec fails does the rule fall through to CONFIRMED / weakly_consistent / partial / REJECTED) per audit L4.8/PE.4 precision; pins the implementation choice on `bout_n_per_day` buildup-headline NULL verdict; (4) **result.md §4 sens_F nonlinear-time surfacing** — three sens_F cells reach p<0.05 (recovery_half_life buildup +3.84 p=0.032; AUC_above_baseline buildup +335.95 p=0.0003; bout_n_per_day afbouw −0.196 p=0.003) surfaced as L3.5-discriminating evidence that the linear time covariate may under-fit within-window trend; does not change any verdict; (5) **result.md §5 Holm table footnote** clarifying per-day aggregations excluded from Holm family per L3.3; (6) **result.md §4 bout_n_per_day sens_E numerics** corrected from β=−0.067 |Δβ|=0.035 threshold=0.060 to **β=−0.108 |Δβ|=0.005 threshold=0.113** (CSV truth; crash-distortion-flag-does-not-fire conclusion stands). All six absorbs are mechanical wording / numerics reconciliations against the CSV source-of-truth; no architectural change, no falsification-bar change, no new statistical machinery. **LOCKED 2026-06-23** by user acceptance + §3.6 compression of the audit's PASS-with-caveats with explicit-compression-eligibility verdict. The audit explicitly noted "r4 absorb is purely mechanical compression per `hypothesis_lock_process.md §3.6`"; this r4 honours that closure. |
