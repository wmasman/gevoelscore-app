...
| `bb_overnight_gain` | derived | computed in build script | `bb_sleep_end_value - bb_sleep_start_value` | int | BB units | **first non-null 2024-09-18** → today (matches `bb_sleep_end_value`; 593 / 1755 = 33.8%) | NaN when either input missing; structurally absent pre-2024-09-18 | **Wiggers D2 directly** (BB overnight charge → next-day gevoelscore). Positive = net charge during sleep; negative = drain. **Coverage start verified 2026-06-14 in Session C** — any pre-vs-post analysis on a pre-2024-09-18 boundary returns NaN for this column. Worked example: [`methodology/intervention_effects_descriptive.md` §2b](methodology/intervention_effects_descriptive.md#2b-channel-coverage-gap--bb_overnight_gain). |
