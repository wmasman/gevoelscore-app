...
| `recovery_phase` | derived | computed from `date` (helper: `lc_recovery_phase(d)` per [`methodology/lc_recovery_phase_axis.md §2.1`](methodology/lc_recovery_phase_axis.md)) | — | category | `pre_illness_healthy` (2021-08-16 → 2022-03-20), `acute_infection` (2022-03-21 → 2022-04-03), `lc_pre_ergo` (2022-04-04 → 2022-09-21), `pacing_pre_citalopram_learning` (2022-09-22 → 2022-11-16, ~56 days), `pacing_habit_established` (2022-11-17 → 2024-04-08, ~508 days), `citalopram_modulated` (>= 2024-04-09) | full | never null | **The 6-phase LC recovery axis (added 2026-06-22 at `e00df27`; canonical MD `lc_recovery_phase_axis.md` LOCKED 2026-06-19 at `d47e0d3`).** Default canonical for descriptive work; opt-in for HA pre-regs per `lc_era_temporal_segmentation §6` sub-boundary criteria. Nested over `citalopram_phase` axis at phase 5 (`citalopram_modulated`); for the cross-classification use both columns. 4a/4b sub-boundary at 2022-11-17 derives from user §7b lived-experience 8-week-post-ergotherapie rule (the habit-established transition; not effectiveness). For collapsibility rules (when sub-phases may be pooled within HA / descriptive analysis), see [`methodology/phase_axis_collapsibility_conventions.md`](methodology/phase_axis_collapsibility_conventions.md) (r1 drafted 2026-06-22 at `1282d9b`; LOCK pending audit + r2). |
...
**Canonical operand-family reference (added 2026-07-15)**: for
sleep-derived operands (Families A / B / C / D / E in the catalogue),
consult [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md).
That MD locks the family taxonomy, the definitional-pair discipline
(e.g. `sleep_efficiency_tib` vs `sleep_efficiency_staged`), the
PRIMARY / SENSITIVITY / DEFERRED tier system, and per-operand zero-vs-NaN
policy. New sleep-related columns landing on `per_day_master.csv`
should update `sleep_metrics.md` (family placement, tier, coverage)
in the same commit as the row is added here.
...
| `sleep_deep_min` | daily_computed | `processed/garmin/sleep_extras_daily.csv` | `sleep_deep_min` | float64 | minutes | 2021-08-16 → today (1713 / 1755 = 98.8%) | NaN on gap | Wave 3 add 2026-06-12. Wiggers F2 (deep-sleep deviation ↔ gevoelscore). Health API: "time in seconds the user spent in deep sleep during the sleep period" (converted to minutes at extraction; same phrasing for light/awake stages). |
| `sleep_light_min` | daily_computed | `processed/garmin/sleep_extras_daily.csv` | `sleep_light_min` | float64 | minutes | same | NaN on gap | Wave 3 add 2026-06-12. Health API: "time in seconds the user spent in light sleep during the sleep period." **Superseded note (corrected 2026-07-15)**: the Wave-3 claim that no REM channel exists on the Forerunner 245 was wrong — see `sleep_rem_min` below; light sleep is a distinct stage, not a REM catch-all. |
| `sleep_rem_min` | daily_computed | `processed/garmin/sleep_extras_daily.csv` | `sleep_rem_min` | float64 | minutes | 2021-08-16 → today (1685 / 1755 = 96.0%) | NaN on gap and on the ~2.8% of nights where Garmin used the deep/light/awake-only classifier (no `remSleepSeconds` emitted) | **Added 2026-07-15** — corrects the earlier Wave-3 assertion that the FR245 (Elevate V3) produces no REM classification. `remSleepSeconds` is in fact present on 1685 nights (verified 2026-07-15: nonzero every year 2021-2026), so a real REM stage IS available. Health API: "time in seconds the user spent in REM sleep during the sleep period." Enables the REM-specific parts of Wiggers F2. |
| `sleep_awake_min` | daily_computed | `processed/garmin/sleep_extras_daily.csv` | `sleep_awake_min` | float64 | minutes | same | NaN on gap | Wave 3 add 2026-06-12. Within-night awake time. Health API: "time in seconds the user spent awake during the sleep period." |
| `sleep_unmeasurable_min` | daily_computed | `processed/garmin/sleep_extras_daily.csv` | `sleep_unmeasurable_min` | float64 | minutes | same | NaN on gap | Wave 3 add 2026-06-12. Time the device could not classify (off-wrist, motion, sensor dropout). High values → low confidence in the stage breakdown for that night. Health API: "time in seconds that the sleep level of the user could not be measured." |
...
| `sleep_hr_avg_spo2` | daily_computed | `processed/garmin/sleep_extras_daily.csv` | `sleep_hr_avg_spo2` | float64 | bpm | 2021-08-16 → today (LC-era 1417 / 1524 = 93.0%) | NaN when the `spo2SleepSummary` JSON block is absent OR when `averageHR` is missing from the block. Not correlated with the Pulse Ox toggle (Garmin computes HR even when SpO2 is disabled). | **Added 2026-07-15** (Stage 3a sleep-operand family). Overnight-average HR reported inside the `spo2SleepSummary` block. Cleaner overnight-HR channel than any FIT re-derivation: Garmin has already used their own sleep-window bounds + sample-quality filters. Directly relevant to Radin 2024's Long-COVID nightly-HR persistence finding (see [`literature/reviews/pem_recovery_trajectory_review.md`](literature/reviews/pem_recovery_trajectory_review.md)). Definitional-pair companion to daily `resting_hr` and waking `hr_median_waking`; pick one primary per hypothesis. Canonical operand family reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §7. |
| `sleep_window_confirmation_type` | daily_computed | `processed/garmin/sleep_extras_daily.csv` | `sleep_window_confirmation_type` | category | enum: `ENHANCED_CONFIRMED`, `ENHANCED_CONFIRMED_FINAL`, `AUTO_CONFIRMED_FINAL`, `AUTO_CONFIRMED`, `UNCONFIRMED`, `OFF_WRIST` | 2021-08-16 → today (LC-era 1503 / 1524 = 98.6%) | NaN when the enum field is absent from the JSON | **Added 2026-07-15** (Stage 3a sleep-operand family). Garmin's own sleep-window validity flag. LC-era distribution: `ENHANCED_CONFIRMED` 990, `ENHANCED_CONFIRMED_FINAL` 466, `UNCONFIRMED` 20, `AUTO_CONFIRMED_FINAL` 17, `OFF_WRIST` 9, `AUTO_CONFIRMED` 1, missing 21. **Downstream discipline**: for analytical use, filter to `ENHANCED_*` + `AUTO_*` (retains 1474 / 1524 = 96.7% LC-era rows); exclude `OFF_WRIST` + `UNCONFIRMED` + missing. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §4.3. |
| `sleep_efficiency_tib` | derived | computed in `pipeline/03_consolidate/build_unified_dataset.py` | `(sleep_duration_min - sleep_awake_min) / sleep_duration_min` | float64 | fraction 0-1 | LC-era 1474 / 1524 = 96.7% | NaN when either input NaN OR `sleep_duration_min == 0` (device-side artefact — Garmin recorded no sleep on that date; not a zero-efficiency observation) | **Added 2026-07-15** (Stage 3a sleep-operand family). **CANONICAL sleep efficiency** per Miller 2022 / standard sleep-medicine (time-asleep / time-in-bed). Corpus median 0.995, mean 0.991, range 0.796-1.000. Definitional-pair discipline (CONVENTIONS §3.3) with `sleep_efficiency_staged`: pick one per analysis; `_tib` is the PRIMARY default. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §4.4. |
| `sleep_efficiency_staged` | derived | computed in `pipeline/03_consolidate/build_unified_dataset.py` | `(sleep_deep_min + sleep_light_min + sleep_rem_min) / sleep_duration_min` | float64 | fraction 0-1 | LC-era 1455 / 1524 = 95.5% (inherits REM coverage) | NaN when any input NaN OR `sleep_duration_min == 0` | **Added 2026-07-15** (Stage 3a sleep-operand family). SENSITIVITY variant of `sleep_efficiency_tib`. Uses classified-asleep minutes as numerator instead of TIB-minus-awake; differs from `_tib` when `sleep_unmeasurable_min` is non-trivial. Corpus median 0.994. Definitional-pair with `sleep_efficiency_tib`; report `_staged` as sensitivity read only if primary verdict is sensitive to the choice. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §4.4. |
| `sleep_rem_frac` | derived | computed in `pipeline/03_consolidate/build_unified_dataset.py` | `sleep_rem_min / (sleep_deep_min + sleep_light_min + sleep_rem_min)` | float64 | fraction 0-1 | LC-era 1456 / 1524 = 95.5% | NaN when any input NaN OR when the denominator (classified-asleep sum) is zero (device-side artefact) | **Added 2026-07-15** (Stage 3a sleep-operand family). REM share of classified-asleep minutes. Corpus median 0.174, mean 0.179, range 0.008-0.396. Architecture-share operand: less sensitive to total-duration variation than raw REM minutes; downstream analyses pick whichever answers the question of interest. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §4.5. |
| `sleep_deep_frac` | derived | computed in `pipeline/03_consolidate/build_unified_dataset.py` | `sleep_deep_min / (sleep_deep_min + sleep_light_min + sleep_rem_min)` | float64 | fraction 0-1 | LC-era 1456 / 1524 = 95.5% | NaN when any input NaN OR when the denominator is zero | **Added 2026-07-15** (Stage 3a sleep-operand family). Deep-sleep share of classified-asleep minutes. Corpus median 0.092, mean 0.098, range 0.000-0.380. Architecture-share companion to `sleep_rem_frac`. Light-sleep fraction is derivable as `1 - sleep_rem_frac - sleep_deep_frac` and is not stored separately. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §4.5. |
| `sleep_waso_frac` | derived | computed in `pipeline/03_consolidate/build_unified_dataset.py` | `sleep_awake_min / sleep_duration_min` | float64 | fraction 0-1 | LC-era 1474 / 1524 = 96.7% | NaN when either input NaN OR `sleep_duration_min == 0` | **Added 2026-07-15** (Stage 3a sleep-operand family). WASO fraction of TIB. Corpus median 0.005 (most nights have very low awake-time). Definitional complement to `sleep_efficiency_tib` (`sleep_waso_frac + sleep_efficiency_tib == 1` by construction on nights where both are computable); SENSITIVITY tier. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §4.6. |
...
**Field semantics (documentation lookup + corpus verification, 2026-07-15)**:
the UDS `bodyBattery` / `allDayStress` objects mirror Garmin Connect's
*internal* wellness-service schema, which Garmin does not publicly
document. The nearest official source is the Garmin Health API
specification (v2.9.6, mirrored verbatim in the MyDataHelps Garmin
export-format docs), whose daily-summary definitions map 1:1 onto these
fields; per-row notes below cite it as "Health API". Corpus-wide
verification over all 1734 UDS dates in
`DI-Connect-Aggregator/UDSFile_*.json` (one-off scratchpad script,
2026-07-15): the stress aggregators are built from **1-minute epochs**
— `totalStressCount × 60 == totalDuration` on every day, and each epoch
is either valid (`stressIntensityCount` → `stressDuration` +
`restDuration`), off-wrist (`stressOffWristCount` →
`uncategorizedDuration`), or too-active (`stressTooActiveCount` →
`activityDuration`); TOTAL == AWAKE + ASLEEP holds for every count,
duration, and intensity field on all 1734 days. Official epoch-level
sentinel codes (Health API): -1 OFF_WRIST, -2 LARGE_MOTION, -3
NOT_ENOUGH_DATA, -4 RECOVERING_FROM_EXERCISE, -5 UNIDENTIFIED — the
§7B daily-level negative sentinels are these codes surfacing in the
aggregate. Body Battery scale is 5-100 (5 is the algorithm floor,
visible as the corpus minimum). BB stat availability across the corpus
(named counts, of 1732 days with a `bodyBattery` object): HIGHEST /
LOWEST / MOSTRECENT 1732, ENDOFDAY 1672, STARTOFDAY 1231, DURINGSLEEP
863 (first 2023-12-18), SLEEPSTART 667 (first 2024-07-08), SLEEPEND 593
(first 2024-09-18) — the two-stage rollout documented on
`bb_overnight_gain`. `bodyBatteryStatus` enum in corpus: MEASURED 10113
/ MODELED 56 / RESET 53 of 10222 stat entries (MODELED = estimated
across data gaps, RESET = algorithm restart — community-documented, not
official). The unextracted UDS fields `averageStressLevelIntensity` /
`totalStressIntensity` remain **undocumented anywhere and semantically
unstable** (their sign conventions differ between the AWAKE and ASLEEP
aggregators in this corpus); do not build metrics on them.

### Body Battery (11 columns)

| name | class | source | source column | dtype | units | coverage | missingness | notes |
|---|---|---|---|---|---|---|---|---|
| `bb_charged_24h` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `bb_charged_24h` | int | BB units (0-100 scale) | 2021-08-16 → today (1732 / 1755 = 98.7%) | NaN on gap | Total BB recharge over the calendar day. Official (Garmin support FAQ): charged/drained are tallied over the 24-h period starting and ending at midnight; charge accrues during sleep and daytime rest, so this can exceed `bb_overnight_gain` (e.g. 58 vs 54 on 2026-02-19). |
| `bb_drained_24h` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `bb_drained_24h` | int | BB units | same | NaN on gap | Total BB drain over the calendar day (same midnight-to-midnight window; drain from activity and stress). Wiggers D4 (BB declines steeply around crashes). |
...
| `bb_during_sleep_value` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `bb_during_sleep_value` | int | BB units | **first non-null 2023-12-18** → today (~861 / 1755 after the 2026-06-12 sentinel filter dropped 2 negative values) | NaN if not aligned **OR** sentinel-filtered (see policy note above). Affected dates: 2024-02-19 (-4 → blank), 2024-07-16 (-5 → blank). Pre-2023-12-18 structurally absent. | **Net BB change across the sleep window** (Garmin `DURINGSLEEP` stat) — NOT a peak; the earlier description "BB peak during the sleep window" was wrong (corrected 2026-07-15). **Layer 3 rho=+1.000 with `bb_overnight_gain` resolved as DEFINITIONAL (2026-07-15)**: `DURINGSLEEP == SLEEPEND − SLEEPSTART` exactly on all 593 corpus days where the three stats coexist — the two columns are the same quantity. DURINGSLEEP exists on 863 days (from 2023-12-18), i.e. before SLEEPSTART/SLEEPEND were exported, so for 2023-12-18 → 2024-09-17 it is the only exported form of the overnight-gain quantity. Treat as one signal in regression / VAR / CCF (pick `bb_overnight_gain` for Wiggers D2 alignment). **Coverage start verified 2026-06-14 in Session C**. |
...
| `bb_overnight_recovery_rate` | derived | computed in `pipeline/03_consolidate/build_unified_dataset.py` | `bb_overnight_gain / (sleep_duration_min / 60)` | float64 | BB units per hour of sleep | LC-era 593 / 1524 = 38.9% (inherits `bb_overnight_gain` truth-source sparsity) | NaN when `bb_overnight_gain` NaN OR `sleep_duration_min` NaN OR `sleep_duration_min == 0` | **Added 2026-07-15** (Stage 3a sleep-operand family). Duration-normalised BB recharge: BB points per hour of sleep. Corpus median 4.81, mean 4.77, range 0.586-8.975. **Sourced from `bb_overnight_gain` (truth) not `bb_overnight_gain_best`** — separate `_best`-sourced variant can be added later if a downstream MD justifies the ~5pp extra coverage. Firstbeat-opacity caveat + no-PEM-anchor caveat: SENSITIVITY tier at most. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §8.2. |
| `bb_overnight_gain_frac` | derived | computed in `pipeline/03_consolidate/build_unified_dataset.py` | `bb_overnight_gain / (100 - bb_sleep_start_value)` | float64 | fraction 0-1 (headroom recharged) | LC-era 593 / 1524 = 38.9% (inherits `bb_overnight_gain` truth-source sparsity) | NaN when `bb_overnight_gain` NaN OR `bb_sleep_start_value` NaN OR `bb_sleep_start_value == 100` (degenerate ceiling case) | **Added 2026-07-15** (Stage 3a sleep-operand family). Ceiling-corrected BB recharge: fraction of available headroom recharged. Corpus median 0.690, mean 0.680, range 0.046-1.000. Sourced from `bb_overnight_gain` (truth) — same source rationale as `bb_overnight_recovery_rate`. Firstbeat-opacity caveat + no-PEM-anchor caveat: SENSITIVITY tier at most. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §8.3. |
...
| name | class | source | source column | dtype | units | coverage | missingness | notes |
|---|---|---|---|---|---|---|---|---|
| `all_day_stress_avg` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `all_day_stress_avg` | int | Garmin stress 0-100 (TOTAL aggregator) | 2021-08-16 → today (~1732 / 1755 after the 2026-06-12 sentinel filter dropped 2 negative values) | NaN on gap **OR** sentinel-filtered (see §7B policy note). Affected dates: 2021-08-16 (-2), 2022-10-28 (-1). | 24-hour mean stress level over **valid 1-minute epochs only** — off-wrist and too-active minutes carry no stress value and are excluded from the denominator. Verified 2026-07-15: TOTAL avg reproduces as the `stressIntensityCount`-weighted mean of AWAKE/ASLEEP within ±1 on 1722/1722 evaluable days, and FAILS when weighted by `totalStressCount` (914 days off) — a single-day spot-check had suggested the latter; the corpus check rejected it. Health API: "an abstraction of the user's average stress level in this monitoring period, measured from 1 to 100, or -1 if there is not enough data to calculate average stress" (matches the -1 sentinel policy above). The stress score itself is the Firstbeat HRV-based 0-100 scale: rest 1-25, low 26-50, medium 51-75, high 76-100. Wiggers C2 (high daily stress → worse next-day recharge), C3 (non-linear stress→fatigue relationship). |
| `all_day_stress_max` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `all_day_stress_max` | int | same | same | NaN on gap. *Not sentinel-filtered* — `0` values on the two whole-day-void dates (2021-08-16, 2022-10-28) are left raw because the day is already flagged by the avg-channel sentinel. | 24-hour peak stress. Health API: "the highest stress level measurement taken during this monitoring period." **Layer 3 rho=+1.000 with `awake_stress_max` resolved as DEFINITIONAL (2026-07-15)**: TOTAL `maxStressLevel == max(AWAKE, ASLEEP)` on 1723/1723 evaluable corpus days, and equals the AWAKE max on 1713/1723 — the two columns are the same measurement except on the 10 days where the sleep-window peak exceeded the waking peak. Treat as one signal in regression / VAR / CCF (pick `all_day_stress_max` for broader frame). See memory `feedback_definitional_pair_guardrail`. |
| `awake_stress_avg` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `awake_stress_avg` | int | same (AWAKE aggregator) | same (~1732 / 1755 after the same sentinel filter) | same dropped dates as `all_day_stress_avg` | Waking-period mean stress; separates daytime sympathetic state from sleep-window stress. Valid waking epochs only — same denominator semantics as `all_day_stress_avg`. |
...
| `asleep_stress_avg_uds` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `asleep_stress_avg_uds` | int | same (ASLEEP aggregator) | 2021-08-16 → today (~1722 / 1755 after the 2026-06-12 sentinel filter dropped 12 negative values) | NaN on gap **OR** sentinel-filtered. 12 affected dates total: 2 whole-day Pattern A + 10 days where Garmin's UDS asleep_stress channel failed (8 have `sleep_valid_flag=False`, 2 (2025-11-19, 2025-11-24) have valid FIT sleep — these are Garmin internal-channel discrepancies; use `stress_mean_sleep` for those nights). | UDS-aggregated sleep-window mean stress. **Different source from `stress_mean_sleep`** (which is recomputed from raw FIT stress samples by `extract_sleep_stress.py`). Cross-check: they should agree closely; documented divergence usually means sleep-window boundary differences between UDS and FIT extraction. Valid sleep-window epochs only — same denominator semantics as `all_day_stress_avg`. |
| `asleep_stress_max_uds` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `asleep_stress_max_uds` | int | Garmin stress 0-100 (ASLEEP aggregator) | 2021-08-16 → today (LC-era 1492 / 1524 = 97.9%) | NaN on gap OR sentinel-filtered per §7B policy | **Added 2026-07-15** (Stage 3a sleep-operand family). UDS-aggregated sleep-window peak stress; arousal-peak proxy. Corpus median 52, mean 53.6, range 20-99. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §7.1. |
| `asleep_stress_high_min_uds` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `asleep_stress_high_min_uds` | float64 | minutes | 2021-08-16 → today (LC-era **156 / 1524 = 10.2%**) | NaN when Garmin's ASLEEP aggregator did not emit the `highDuration` field. The 90% "missing" rate is Garmin's intentional emit-only-when-nonzero behaviour — the field is absent from the JSON on nights without high-band stress minutes. | **Added 2026-07-15** (Stage 3a sleep-operand family). Minutes of ASLEEP stress in the high band (`highDuration / 60`). **CRITICAL zero-vs-NaN semantics**: missing IS NOT the same as zero — on the 90% of days where the field is absent, we know Garmin did not emit high-band minutes but cannot distinguish "genuinely zero" from "aggregator declined to score" cases from the JSON alone. NaN at extraction; downstream consumers who choose `.fillna(0)` must justify at analysis time. Corpus (on the 10.2% of days with data): median 1.0 min, max 257 min. Canonical reference: [`methodology/sleep_metrics.md`](methodology/sleep_metrics.md) §7.3. |

### 24h / waking respiration (3 columns)

| name | class | source | source column | dtype | units | coverage | missingness | notes |
|---|---|---|---|---|---|---|---|---|
| `respiration_avg_waking` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `respiration_avg_waking` | float64 | breaths/min | 2021-08-16 → today (1732 / 1755 = 98.7%) | NaN on gap | Waking-period mean respiration rate. Wiggers G1 waking variant. Official docs define respiration only at the epoch level ("respiration measurement is in breaths per minute"); the daily `avgWakingRespirationValue` / `highestRespirationValue` / `lowestRespirationValue` rollups are Connect-internal names (mean over waking epochs; 24-h extremes). |
...
| name | class | source | source column | dtype | units | coverage | missingness | notes |
|---|---|---|---|---|---|---|---|---|
| `spo2_avg_24h` | daily_computed | `processed/garmin/uds_extras_daily.csv` | `spo2_avg_24h` | float64 | percent | 2021-08-16 → today (1688 / 1755 = 96.2%) | NaN on gap; lower fill where Pulse Ox toggle was off | 24-hour mean SpO2. Wiggers G4 (deprioritised). UDS `averageSpo2Value`; official Pulse Ox export docs define only the epoch series (SpO2 percentages), the daily rollups are Connect-internal. |
...
### Section 8E — Bout-level per-day aggregations (5 + 5 = 10 columns; first 5 added 2026-06-22 at `d5b394c`; last 5 added 2026-07-09 at Bundle H+ event 7)

**Section preamble**: per-day summaries of per-bout features extracted by [`pipeline/02_features/extract_stress_bouts.py`](pipeline/02_features/extract_stress_bouts.py) per the locked methodology MD [`methodology/bout_level_recovery_dynamics.md`](methodology/bout_level_recovery_dynamics.md) (LOCKED r3 2026-07-09 with §3.2.2 SD-anchored derivative operand family additive extension). The per-bout dataset itself lives external at `$GEVOELSCORE_DATA_PATH/unified/per_bout_master.csv` (4,317 bouts across 1,479 valid days; bout-detection rule = peak ≥60 + 50-floor hysteresis + 5-min minimum + 60-min separation + 180-min return-window cap). The **first 5 columns** are the **Moderate aggregation set** ratified at PM AskUserQuestion 2026-06-22 Decision 2. The **last 5 columns** are the **Personal-baseline SD-anchored derivative operand family** ratified at Bundle H+ event 7 (2026-07-09) co-lock with HA-C4cp r2 pre-reg per parent MD §3.2.2 LOCKED r3 pre-commits.

**For per-bout column-level definitions** (the 25 columns of `per_bout_master.csv` — `peak_height`, `recovery_half_life`, `decay_slope`, `tail_length`, `AUC_above_baseline`, the 8 flags, the 3 citalopram covariates, etc.), **see [`pipeline/02_features/README.md` §Per-bout CSV columns](pipeline/02_features/README.md)** (canonical extraction schema) and [`methodology/bout_level_recovery_dynamics.md` §3-§4](methodology/bout_level_recovery_dynamics.md) (definition spec + NaN semantics). DATA_DICTIONARY.md is scoped to `per_day_master.csv` per §0 above; `per_bout_master.csv` is a separate external dataset with its own canonical home.
...
| `subject_lagged_median_day` | daily_computed | `$GEVOELSCORE_DATA_PATH/unified/per_bout_master.csv` via aggregation | derived (median `tail_length` over reference pool of bouts on days in `[d-90, d-30]` per MD §3.2.2) | float64 | minutes | 1450 / 1486 valid-day coverage on Stratum 4 (leftmost ~36 LC-era days lack enough reference-pool bouts) | NaN on days where `[d-90, d-30]` reference pool has < 30 bouts (parent MD §3.2.2 validity bar) OR §3.4 day-validity gate fails | **Reference-window audit trace** for the SD-anchored derivative operand family. Cross-phase-pooled Stratum 4 median-of-medians ≈ 46.5 min; range [24, 87]. Enables auditing whether an individual day's Z-score is anchored to a reasonable reference. Load-bearing zero-vs-NaN discipline: never `.fillna(0)` — NaN means reference window insufficient, not "median = 0". |
| `subject_lagged_mad_day` | daily_computed | `$GEVOELSCORE_DATA_PATH/unified/per_bout_master.csv` via aggregation | derived (`1.4826 × MAD` of reference pool `tail_length` per MD §3.2.2) | float64 | minutes | same as `subject_lagged_median_day` | same NaN semantics | **Reference-window audit trace** for the SD-anchored derivative operand family. Normal-consistent MAD scaling per CONVENTIONS §3.1 robust-baseline prototype. Cross-phase-pooled Stratum 4 median-of-MADs ≈ 57.8 min. The high MAD reflects the right-censored `tail_length` distribution (180-min cap pile-up). |
| `bout_return_time_z_max_day` | daily_computed | `$GEVOELSCORE_DATA_PATH/unified/per_bout_master.csv` via aggregation | derived (max over day's bouts of `bout_return_time_z` per MD §3.2.2) | float64 | SD-equivalents | 1269 / 1274 on primary Stratum 4 (`>= 2022-11-17`) | NaN on reference-window-invalid days OR days with 0 bouts | Highest z-scored return-time bout per day. Continuous alternative to the Z=1 / Z=2 binary flags; used as HA-C4cp §4.10 continuous sensitivity arm. Cross-phase-pooled mean ≈ 1.63; median ≈ 1.73. |
| `bout_n_did_not_return_1sd_day` | daily_computed | `$GEVOELSCORE_DATA_PATH/unified/per_bout_master.csv` via aggregation | derived (count of bouts with `did_not_return_1sd_flag = True` per MD §3.2.2) | int | bouts/day | same as `bout_return_time_z_max_day` | **`0` on valid days with valid reference window + no flagged bouts; NaN on reference-window-invalid days OR §3.4-invalid days** | Personal-baseline Z=1 tail count on the SD-anchored operand family. Threshold: `tail_length > subject_lagged_median + 1 × subject_lagged_mad`. Cross-phase-pooled Stratum 4 mean ≈ 0.93. Used as HA-C4cp §4.10 Z=1 sensitivity arm. **Load-bearing zero-vs-NaN discipline per parent MD §4**: `.fillna(0)` would fold reference-window-invalid days into zero-count days and bias downstream contrasts toward null. |
| `bout_n_did_not_return_2sd_day` | daily_computed | `$GEVOELSCORE_DATA_PATH/unified/per_bout_master.csv` via aggregation | derived (count of bouts with `did_not_return_2sd_flag = True` per MD §3.2.2) | int | bouts/day | same | same zero-vs-NaN discipline | **HA-C4cp primary per-day operand** per pre-reg `hypothesis.md` r2 LOCKED 2026-07-09. Personal-baseline Z=2 tail count. Threshold: `tail_length > subject_lagged_median + 2 × subject_lagged_mad`. Cross-phase-pooled Stratum 4 mean ≈ 0.48; median 0; breakdown 786 zero + 488 ≥1 + 4 ≥3 flagged bouts. **Cross-op-independent** from `bout_n_did_not_return` (fixed 180-min absolute threshold) per parent MD §3.2.2 operand-family independence claim: personal-baseline-rolling vs fixed-absolute-threshold reference frames capture different bout populations. At days with high `subject_lagged_mad_day`, Z=2 threshold may exceed the 180-min `tail_length` cap; no bout can qualify on those days by construction. |

**Cross-references**:
- Methodology spec: [`methodology/bout_level_recovery_dynamics.md`](methodology/bout_level_recovery_dynamics.md) §2 (bout-detection rule), §3 (per-bout feature set), §3.2.2 (SD-anchored derivative operand family LOCKED r3 2026-07-09), §4 (per-day aggregations), §6 (framework-validity gate + HALT rule)
- Downstream HA pre-reg using the SD-anchored family: [`analyses/hypotheses/HA-C4cp/hypothesis.md`](analyses/hypotheses/HA-C4cp/hypothesis.md) r2 LOCKED 2026-07-09 (primary operand `bout_n_did_not_return_2sd_day`; sensitivity Z=1 + z_max continuous arms)
...
| 2026-07-15 | Garmin UDS field-semantics documentation + two rho=1.000 pairs resolved | Looked up official Garmin Health API definitions (Health REST API spec 2.9.6, mirrored in the MyDataHelps export-format docs + Garmin support FAQs) for the raw UDS `bodyBattery` / `allDayStress` / sleep-stage fields, and verified the identities against all 1734 UDS dates (scratchpad script). Added a **field-semantics preamble to §7B** (1-min epoch model, TOTAL == AWAKE + ASLEEP, epoch sentinel codes, BB 5-100 scale, corpus stat-availability counts, `bodyBatteryStatus` enum MEASURED/MODELED/RESET). **Resolved two of the three Layer-3 rho=1.000 definitional-vs-empirical pairs** flagged 2026-06-12: (1) `bb_during_sleep_value` (DURINGSLEEP) == `bb_overnight_gain` (SLEEPEND−SLEEPSTART) exactly on all 593 co-occurring days — **DEFINITIONAL**; also corrected its description from the wrong "BB peak during sleep" to "net BB change across the sleep window". (2) `all_day_stress_max` (TOTAL `maxStressLevel`) == max(AWAKE, ASLEEP `maxStressLevel`) on 1723/1723 days, and == the extracted `awake_stress_max` on 1713 (differs only the 10 days a sleep-window peak exceeded the waking peak) — **DEFINITIONAL** (ASLEEP max is present in raw JSON but not extracted as a column). Documented that `all_day_stress_avg` is a valid-epoch-weighted mean (weighting by `stressIntensityCount` reproduces TOTAL on 1722/1722 days; `totalStressCount` fails on 914) — corrects a single-day spot-check inference. Added official stage-duration definitions to §7 sleep rows. **No column added/removed; corrections + provenance only.** |
| 2026-07-15 | `sleep_rem_min` documented (corrects Wave-3 no-REM claim) | The Wave-3 `sleep_light_min` note asserted the Forerunner 245 (Elevate V3) produces no REM-stage classification and that all non-deep sleep falls into light. That was **wrong**: `remSleepSeconds` is present on 1685 / 1755 nights (96.0%, nonzero every year 2021-2026) in `sleepData.json`, confirmed by the official Health API sleep field "time in seconds the user spent in REM sleep during the sleep period". The pipeline change that added `sleep_rem_min` (`garmin_sleep_extras.py` + `build_unified_dataset.py`, in-flight this session) is documented here; §7 gains a `sleep_rem_min` row and the `sleep_light_min` note is superseded. |
