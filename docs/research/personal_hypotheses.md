...
5. **Intervention-baseline-step-changes — narrowed by [`intervention_effects_descriptive.md §8`](methodology/intervention_effects_descriptive.md#8-findings-session-c-run-2026-06-14) (Session C run 2026-06-14).** The rolling `[d-90, d-30]` baseline does not segment around documented interventions, but the corpus-wide descriptive sweep ruled out a corpus-wide M2 boundary and identified exactly **two channel×boundary pairs** with detrend-surviving step-changes: (a) `resting_hr` around 2022-09-22 Ergotherapie start, large but confounded with the steep LC trajectory's deterioration phase (causal attribution unsupported); (b) `stress_mean_sleep` around 2026-03-20 Citalopram afbouw start, mechanistically clean SSRI-withdrawal candidate (pending dose-response follow-up MD). **For P4a specifically**: BB-floor-crossing is the predictor, and `bb_lowest` (a sibling channel) did NOT show detrend-surviving step-changes at any boundary; P4a's BB-based predictor is unlikely to be intervention-confounded. The caveat is now narrower than the original blanket flag.
...
6. **Intervention-baseline-step-changes — narrowed by [`intervention_effects_descriptive.md §8`](methodology/intervention_effects_descriptive.md#8-findings-session-c-run-2026-06-14)** (Session C run 2026-06-14). See P4a caveat 5 for the full narrowing. **For P4b specifically**: the predictor is a rolling-baseline z-score on `bb_overnight_gain` and similar BB-trajectory metrics. The Session C sweep showed `bb_overnight_gain` had its own coverage limitation (data starts 2024-09-18 — see [§2b](methodology/intervention_effects_descriptive.md#2b-channel-coverage-gap--bb_overnight_gain)) and the one analyzable boundary (2026-03-20) showed a step that the detrend revealed as a recovery-trajectory artifact, NOT an intervention effect. P4b's BB-trajectory predictor on post-2024-09-18 data is therefore unlikely to be intervention-confounded; pre-2024-09-18 BB-overnight-gain data is unavailable anyway.
...
6. **Intervention-baseline-step-changes — narrowed by [`intervention_effects_descriptive.md §8`](methodology/intervention_effects_descriptive.md#8-findings-session-c-run-2026-06-14)** (Session C run 2026-06-14). See P4a caveat 5 for the full narrowing. **For P5b specifically**: the predictor combines stress (`stress_mean_sleep` / `all_day_stress_avg`) with motion. The Session C sweep flagged `stress_mean_sleep` as the ONE channel with a detrend-surviving step-change at 2026-03-20 (Citalopram afbouw start). **P5b inherits this caveat sharply**: the rolling-baseline z-score on `stress_mean_sleep` may be miscalibrated for days within ±60 of 2026-03-20 (the SSRI-withdrawal-attributable shift). Sensitivity arm needed: exclude days within the afbouw transition window, or use the pending dose-response narrow MD's segmentation when it lands.
...
5. **Intervention-baseline-step-changes — narrowed by [`intervention_effects_descriptive.md §8`](methodology/intervention_effects_descriptive.md#8-findings-session-c-run-2026-06-14)** (Session C run 2026-06-14). See P4a caveat 5 for the full narrowing. **For P6 specifically**: the post-crash recovery shape is per-channel; the only channel with a detrend-surviving step-change adjacent to P6's observation window (recent crashes in late 2025 / 2026) is `stress_mean_sleep` around the 2026-03-20 Citalopram afbouw boundary. **Crashes whose `t0` falls within ±60 days of 2026-03-20** inherit a recovery-shape caveat for the `stress_mean_sleep` channel only; other channels are unaffected. CONVENTIONS §3.7 trajectory-detrend pattern applies to P6's recovery-shape stats per default.
...
4. **Intervention-baseline-step-changes — narrowed by [`intervention_effects_descriptive.md §8`](methodology/intervention_effects_descriptive.md#8-findings-session-c-run-2026-06-14)** (Session C run 2026-06-14). See P4a caveat 5 for the full narrowing. **For P7 specifically**: the predictor is crash-label-density (not a Garmin baseline channel); the outcome is also a crash-label flag. Intervention-baseline-step-changes operate on the Garmin baseline frame, not directly on crash labels — but they DO operate on `gevoelscore` (the crash-label generator). Session C showed `gevoelscore` had a small detrend-surviving step UP around the 2026-03-20 Citalopram afbouw start; this could subtly shift the crash-label threshold-crossing rate for late-2025 / 2026 crashes. **Operational implication for P7**: report the 14d-window crash density separately for "all LC era" vs "afbouw era (2026-03-20 onward)" as a sensitivity arm.
