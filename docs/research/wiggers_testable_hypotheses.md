...
| **C4** (3-channel stress decay triad) | source-verified; Wave 4 operationalised; decay + walls + t+1 reactivity all three channels measurable. **HA-C4 v2 r2 LOCKED 2026-06-18** (`b0f38a7`) → tested 2026-06-18 (`52bddb5`) → **REJECTED at triad-aggregation level** (validate Ch1+Ch2 SUPPORTED; train REFUTED; daily-aggregate is wrong instrument for within-day-recovery claim — see [`reviews/HA-C4-v2-2026-06-18.md`](reviews/HA-C4-v2-2026-06-18.md) + [`analyses/hypotheses/HA-C4/result.md`](analyses/hypotheses/HA-C4/result.md)). **Bout-level pivot landed via HA-C4c sister pre-reg**: **HA-C4c r2 LOCKED 2026-06-23** at [`analyses/hypotheses/HA-C4c/hypothesis.md`](analyses/hypotheses/HA-C4c/hypothesis.md) (r1 drafted `d59352c` → audit REVISION-RECOMMENDED `5f79bd1` with L1.2 BLOCKING 5-surface column-confusion + L4.7 §4.9 framing slip + L3.3 Holm shrunk-family disclosure → r2 substantive absorb `310e145` + fresh-session re-audit clean per Option γ → r2 LOCKED). Substantive Wiggers C4 retest at bout-level resolution: operand `bout_n_did_not_return_day` per-day count; cross-phase pooled on `citalopram_phase` axis primary + unmedicated-only sensitivity arm; 4 cascade-context caveats baked in (HA11-bout-redo PARTIAL bar-3 calibration discount + β-recalibration dose-naive primary + 99.3% motion-confound corpus property + transient-fragility). | WE | `stress_post_peak_time_to_rest_min`, `stress_high_duration_min`, `awake_stress_avg` ready; `bout_n_did_not_return_day` per-day ready |
