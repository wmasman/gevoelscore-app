...
| C4 | After overexertion, stress fails to drop during rest periods ("stuck sympathetic") | Intraday stress | Needs intraday export; reduced within-day recovery dips on `t+1`. **Formal pre-reg LOCKED 2026-06-18 v2 r2 at [`analyses/hypotheses/HA-C4/hypothesis.md`](analyses/hypotheses/HA-C4/hypothesis.md)** (v1 LOCKED 2026-06-17 → dry-run HALTED on Ch3 validate n=25 < 30 → v2 fresh-session draft 2026-06-18 with §5.3 INCONCLUSIVE-aware verdict rule + §7.3 arithmetic rebuild + §4.11.3 chain-relaxed sensitivity arm → fresh-session §3.4 audit PASS-with-caveats → r2 four side-obs closures → §3.6 compression). 3-channel confirmatory triad (decay / walls / t+1 reactivity) with INCONCLUSIVE-aware verdict bands (3.0 strong / 2.0-2.5 SUPPORTED / 1.0-1.5 PARTIAL / <1.0 REJECTED). |
