...
|---|---|---|---|
| **C3** (non-linear stress → fatigue) | source-verified VERBATIM (PDF 1357-1368); 30 → 40 stress step costs more than it looks | NL | `all_day_stress_avg`, `gevoelscore` ready |
...
- Intervention-baseline-step-changes — narrowed by [`intervention_effects_descriptive.md §8`](methodology/intervention_effects_descriptive.md#8-findings-session-c-run-2026-06-14) (Session C run 2026-06-14). Corpus-wide M2 boundary ruled out; only two channel×boundary pairs survived as candidates. **For C4b specifically**: the predictor combines `all_day_stress_avg` / `stress_mean_sleep` with motion. The Session C sweep flagged `stress_mean_sleep` as the ONE channel with a detrend-surviving step-change at 2026-03-20 (Citalopram afbouw start). **C4b inherits this caveat sharply** because its rolling-baseline rest-stress threshold uses the same `stress_mean_sleep` baseline; days within ±60 of 2026-03-20 inherit a calibration uncertainty. Sensitivity arm at pre-reg: exclude days within the afbouw transition window OR adopt the pending dose-response narrow MD's segmentation once it lands.
