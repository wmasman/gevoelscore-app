...
|---|---|---|---|
| **C3** (non-linear stress → fatigue) | source-verified VERBATIM (PDF 1357-1368); 30 → 40 stress step costs more than it looks. **Formal pre-reg r2 LOCKED 2026-06-23 at [`analyses/hypotheses/HA-C3/hypothesis.md`](analyses/hypotheses/HA-C3/hypothesis.md)** (r1 drafted 2026-06-22 commit `ec99c2f` → fresh-session §3.4 audit PASS-with-caveats commit `8f3f269` → r2 §3.6-compression absorb of L3.4 substantive [spline F bound onto §4.7 block-permutation E[L]=7] + arithmetic side observation [companion contrast vector replaced with textbook orthogonal quadratic `(+2, -1, -2, -1, +2)` per Option A] + 4 drafter Q closures). Framework-validity test of Wiggers' convex stress→fatigue claim on the LC frame: unmedicated × 5-bin (0-20, 20-30, 30-40, 40-60, 60+) × `gevoelscore` bin-mean × {Jonckheere-Terpstra monotonicity (a) + second-difference contrast (b) + natural-cubic-spline non-linearity (c)} × block-permutation null E[L]=7 × 3-condition gated verdict (SUPPORTED iff all 3 MET; PARTIAL iff 2-of-3 MET and no wrong-direction firing; REJECTED otherwise or on any wrong-direction firing). §5.A unmedicated-headline primary + §5.B dose-adjusted cross-phase sensitivity (per [`citalopram_phase_stratification §4`](methodology/citalopram_phase_stratification.md#4-per-channel-inheritance-rules) CONFIRMED-channel inheritance for `all_day_stress_avg`). Test execution is a separate post-lock session. | NL | `all_day_stress_avg`, `gevoelscore` ready |
| **C4** (3-channel stress decay triad) | source-verified; Wave 4 operationalised; decay + walls + t+1 reactivity all three channels measurable | WE | `stress_post_peak_time_to_rest_min`, `stress_high_duration_min`, `awake_stress_avg` ready |
