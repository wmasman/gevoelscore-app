...
2. **Stratum: cross-phase pooled (phase 5 unmedicated + phase 6 citalopram-modulated) as primary**; **unmedicated-only as a primary sensitivity arm** for cross-test consistency with HA11-bout-redo's reference frame. Rationale: per recalibration §6 inheritance table + STOCKTAKE §6 architectural-implications paragraph, cross-phase pooling is permissible at this corpus's bout-level n without §5.A/B/C inheritance violation — 0/7 features CONFIRMED → Approach A is NOT load-bearing → no inheritance binding at the per-bout layer. Pooling gains ~70 day-clusters of n (per STOCKTAKE §6) which is exactly the dimension HA11-bout-redo's bar 3 failed on (n_calm=70/n_crash=11). Unmedicated-only sensitivity arm preserves cross-test comparability with HA11-bout-redo (same stratum). Alternatives considered + rejected as primary: (a) unmedicated-only — preserves cross-test reference frame BUT inherits the same n_crash~11 power constraint that broke HA11-bout-redo's bar 3 in the calibration; HA-C4c would face the same power problem. (b) per-phase stratified (separate verdicts in unmedicated + buildup_post_cpap + consolidation + afbouw) — n-per-phase too thin for bout-level work at this corpus; meta-combine introduces multiplicity. See §4.2.
...
| **Gate 4** — Re-audit clean OR §3.6 compression | r2 re-audit cleanly closed OR compression decision documented in lock-commit message per §3.6 acceptability criteria | [pending audit + r2 + decision] | [to confirm at lock] |
...
**Primary stratum**: cross-phase pooled — `citalopram_phase(d) ∈ {pacing_habit_established (phase 5), citalopram_modulated (phase 6)}` across the LC era 2022-04-04 → present. Pooling phase 5 + phase 6 gains ~70 day-clusters of n per [STOCKTAKE §6 architectural-implications paragraph](../../../STOCKTAKE.md#6-cross-section-synthesis); permissible at this corpus's bout-level n without §5.A/B/C inheritance violation per recalibration §6 (0/7 features CONFIRMED → Approach A NOT load-bearing → no per-channel inheritance binding for bout-level operands at this n).

**Stratum exclusions**:

- Phase 0 (`pre_illness_healthy`), phase 1 (`acute_infection`), phase 2 (`lc_pre_ergo`), phase 3 (`pacing_pre_citalopram_learning`) — pre-LC-era or pre-pacing-stable; out of scope per parent MD §3.4 device-baseline-lag rule + the bout-extraction pipeline's day-validity gate.
- Phase 4a/4b sub-boundary irrelevant for HA-C4c — phase 5 (`pacing_habit_established`) begins at the lived-experience 8-week-post-ergotherapie boundary 2022-11-17 per [`lc_recovery_phase_axis.md`](../../../methodology/lc_recovery_phase_axis.md). HA-C4c primary stratum is phase 5 + phase 6 only.

**Primary sensitivity arm — unmedicated-only stratum**: `citalopram_phase(d) == pacing_habit_established` only (phase 5; LC era 2022-04-04 → 2024-04-08; excluding April 2024 cluster per §6). Mirrors HA11-bout-redo's reference frame for cross-test consistency; reported per §4.10.
...
### 4.9 Approach A inheritance — sensitivity arm only (per Authorship "Locked decisions" item 7)

**Per recalibration §6 inheritance table**: 0/7 features CONFIRMED at this corpus's bout-level n; Approach A is NOT load-bearing for any downstream HA pre-reg at this corpus's bout-level n. **HA-C4c primary outcome is dose-naive.** Approach A is RELEGATED to sensitivity arm per the recalibration §6 architectural-implications paragraph.

**Inheritance-by-analogue framing** (since `bout_n_did_not_return_day` was NOT in the recalibration's 7-feature scope):

- Closest analogue with a sensitivity inheritance: `bout_n_fast_recovery_day` (weakly_consistent verdict per sub-MD §6; buildup-post-CPAP β = −0.056/mg [95% CI −0.145, +0.034]).
- Use that β as the inheritance template, with sign-flip: `bout_n_did_not_return_day` has prior direction +1 (more failures-to-return under elevated sympathetic tone) while `bout_n_fast_recovery_day` has prior direction −1; the inheritance template is `β_template = +0.056/mg` (sign-flipped to match the +1 prior direction; same magnitude).
- Approach A sensitivity arm: `bout_n_did_not_return_day_adj(d) = bout_n_did_not_return_day(d) − β_template × dose_plasma_mg(d) = bout_n_did_not_return_day(d) − 0.056 × dose_plasma_mg(d)`.
- Re-run the §4.6 primary procedure on the dose-adjusted operand; report alongside the primary as a sensitivity arm; explicitly marked as **inheritance-by-analogue** in the result.md.

**Why this is a sensitivity arm, not primary**: per recalibration §6, the recalibration produced no CONFIRMED β at this corpus's bout-level n; the closest-analogue β (`bout_n_fast_recovery_day` weakly_consistent) has a 95% CI that crosses zero; using a CI-crosses-zero β as a load-bearing dose-correction would inject the recalibration's noise into the primary verdict. Keeping Approach A as a sensitivity arm honours the recalibration's NULL/weakly-consistent reading while still providing the dose-aware read for transparency. §8 caveat 3 names this explicitly.

**Sensitivity-of-verdict-to-CI-bounds**: per parent MD §8 caveat 3 + sub-MD §7 caveat 5, the Approach A sensitivity arm is re-run at the CI lower bound (β = −0.145/mg → +0.145/mg template after sign-flip) AND the CI upper bound (β = +0.034/mg → −0.034/mg template after sign-flip) to bracket the dose-adjustment range. If the verdict on the Approach A sensitivity arm flips across the CI range, surface as a β-precision-fragility finding.
...
- **Primary stratum**: only phase 5 (`pacing_habit_established`; 2022-11-17 → 2024-04-08) UNION phase 6 (`citalopram_modulated`; 2024-04-09 onward, excluding April 2024 cluster). Days in earlier phases (pre_illness_healthy / acute_infection / lc_pre_ergo / pacing_pre_citalopram_learning) excluded. Phase 4a/4b sub-boundary out of scope (irrelevant for HA-C4c — phase 5 begins at the 8-week-post-ergotherapie boundary 2022-11-17 per `lc_recovery_phase_axis.md`).
...
10. **Pacing-behaviour confounder** (inherited from HA-C4 v2 §8 + parent MD §2.4 r2 absorb): if the within-day stress pattern this participant generates is mediated by active pacing behaviour (the participant uses Garmin stress as a live pacing signal per [`garmin_pacing_practice.md`](../../../methodology/garmin_pacing_practice.md)), bout-level analysis may not "fix" the daily-aggregate flatness — it may reveal that the pacing behaviour shapes per-bout features too. The cross-phase-pooled stratum partially insulates against this (the pacing-habit-established phase 5 + citalopram-modulated phase 6 span the full LC era; pre-pacing-stable era days are excluded per §6 stratum). The HA-C4c verdict cannot fully eliminate this risk; a SUPPORTED-here-NOT-SUPPORTED-at-HA-C4b shape is consistent with the protective-rather-than-predictive alternative reading from HA-C4b v3 §9.
...
- HA-C4c-v2 reframe with different stratum or different operand candidate. Options: (a) per-phase stratified verdict (separate verdicts in phase 5 + phase 6; meta-combine); (b) `bout_recovery_half_life_median_day` as primary operand instead; (c) restrict the heavy-T classification to `very_heavy` only (more conservative; smaller n); (d) extend the stratum to phases 3+4+5+6 (full LC era).
...
- Load `per_day_master.csv`; filter to primary stratum (cross-phase-pooled: phase 5 + phase 6) × §4.4 day-validity × §4.3 heavy-T classification non-NaN.
...
- Unmedicated-only stratum (phase 5 only).
