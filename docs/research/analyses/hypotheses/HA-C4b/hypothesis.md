...
- **`gesprek-met-M` event-label promotion + `annotations.yaml` umbrella cleanup** (user-flagged 2026-06-17 data hygiene; affects multiple downstream MDs not just HA-C4b). The current per-clause `gesprek met m` mentions should be promoted to proper `event_labels` per-session markers in `annotations.yaml` so the construct is queryable at the event level (3 known unmed dates: 2024-01-26, 2024-03-13, 2024-03-21; user may have more in calendar / therapist records). Concurrently: the existing `event_labels` "mirjam" + "relatie" tags are duplicates (identical 222-day span 2024-04-01 → 2024-11-08) and should be consolidated into a single `levensgebeurtenis` umbrella span per `DATA_DICTIONARY.md §11`; the "coach" event-label tag is scattered without clear definition (270 days, three disconnected periods) and should be removed from `annotations.yaml`. After the cleanup, regenerate `per_day_master.csv` via the `build_unified_dataset.py` pipeline. The HA-C4b v3 result.md addendum can then surface `gesprek-met-M`-proximity flags on surviving episodes with higher fidelity (descriptive only; no v4 trigger for HA-C4b; the pipeline regen is a separate change).
...
- **Coaching / relationship-therapy sessions (`gesprek-met-M`)** — per user clarification 2026-06-17, the construct the user previously named "rouwschop" maps to **the coaching / relationship-therapy sessions with M**. Three specific dates are surfaceable from clause-level notes (`notes-categorized-v24-clauses.csv` keyword "gesprek met m"): **2024-01-26, 2024-03-13, 2024-03-21** — all in unmedicated. Proximity to pooled-unmedicated crash episodes: 2024-01-12 crash is **14 days before** the first known session; 2024-02-15 crash is **20 days after** the 2024-01-26 session and **26 days before** the 2024-03-13 session. None of the pooled-unmedicated 4-day lead-up windows directly overlap a known session date. The clause-level mention frequency likely UNDER-represents actual session frequency (the rest of the 2024-04-01 → 2024-11-08 dense tagging is post-citalopram, outside v3's headline). The v3 test session's per-episode annotation column for `gesprek-met-M` is descriptive-proximity-flag-only (e.g. "within 30 days of a known session date").

  **Data-hygiene note** (user-flagged 2026-06-17; out of scope for v3, queued in Pending follow-ups below): the existing `event_labels` columns for "mirjam" and "relatie" are duplicates of one another (identical 222-day span 2024-04-01 → 2024-11-08) and should be `levensgebeurtenis` umbrella spans rather than event labels per the [`DATA_DICTIONARY.md` §11](../../../DATA_DICTIONARY.md) convention; the "coach" event-label tag is scattered (270 days, no clear definition) and should be removed from `annotations.yaml` during the cleanup. The correct analytical primitive is the per-session `gesprek-met-M` marker, which should be promoted from clause-level mentions to a proper event_labels event (or `levensgebeurtenis` per-session entries) in `annotations.yaml`. This pipeline cleanup is independent of HA-C4b v3 and lives in the queued follow-ups.
