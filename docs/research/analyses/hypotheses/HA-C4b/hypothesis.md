...
- **`annotations.yaml` umbrella + event-label cleanup** (user-flagged 2026-06-17 data hygiene; affects multiple downstream MDs not just HA-C4b; independent of HA-C4b v3 spec). The existing `event_labels` "mirjam" + "relatie" tags are duplicates of one another (identical 222-day span 2024-04-01 → 2024-11-08) and should be consolidated into a single `levensgebeurtenis` umbrella span per `DATA_DICTIONARY.md §11`. The "coach" event-label tag is scattered without clear definition (270 days, three disconnected periods) and should be removed from `annotations.yaml`. The per-session `gesprek-met-M` marker should be promoted from clause-level mentions to proper `event_labels` per-session markers in `annotations.yaml` (3 known unmed dates: 2024-01-26, 2024-03-13, 2024-03-21; user may have more in calendar / therapist records). After the cleanup, regenerate `per_day_master.csv` via the `build_unified_dataset.py` pipeline. Independent of HA-C4b v3 (does not block lock or run); separate documentation / annotations.yaml-cleanup commit.
...
- **§8.x locked descriptive context sub-block added** (closes audit Section-4 item-5 + side observation re user-supplied date routing). Concrete date anchors are written into the v3 spec record at lock-time (NOT at result.md-production-time), held out from the v3 test-execution session via the handoff brief. The dates are derivable from existing master-CSV columns (no privacy-sensitive name strings surfaced; only ISO date boundaries + the umbrella-flag column names). v3 r2 absorbs the named events with empirical date anchors per §8.x.

**r2-pre-lock amendment 2026-06-17** (user-feedback absorption; same revision, additional context per §3.5 *"All r2 changes must be tracked in the Authorship block"*): user clarified that the construct earlier referenced as "rouwschop" maps to the **coaching / relationship-therapy sessions with M** (`gesprek-met-M`); 3 clause-level mention dates surfaced in unmedicated phase (2024-01-26, 2024-03-13, 2024-03-21). User also flagged data-hygiene issues in `annotations.yaml`: the "mirjam" and "relatie" event_labels are duplicates of one another (identical 222-day span; should be a single `levensgebeurtenis` umbrella per `DATA_DICTIONARY.md §11`); the "coach" event-label tag is scattered without clear definition (270 days, three disconnected periods; should be removed); the per-session `gesprek-met-M` marker should be promoted from clause-level to event_labels. This pipeline cleanup is queued in Pending follow-ups; out of scope for v3 (does not change v3's §5.1 falsification bar, headline cell, or any §4 protocol choice).

**r2-post-amendment-cut 2026-06-17** (user direction: "let's do B and not waste our time too much on things that are part of the data interpretation; first we should run the tests"). User selected option (B) from the spec-weight-vs-discipline-cost trade discussed pre-lock: **cut the §8.x locked-date-anchor sub-block entirely**; keep the §8 qualitative pacing-behaviour caveat only; remove §9 commitments to a per-episode annotation table (cat_belasting_* values, era classification, event-proximity flags); remove §10.3 spec requirement for the annotation table. The audit's Section-4 item-5 push for lock-time date elicitation is partially re-priced: the v3 spec accepts the post-result interpretive-flexibility cost in exchange for a slimmer spec; the §8 qualitative caveat + §9 verdict-invariant treatment remain the only spec-level discipline against post-result fitting. The git history preserves the original §8.x date-anchor content at commits `7595a8b` / `a7bf32a` for audit traceability; the cut is a scope reduction not a methodological change. Any pacing-era / event-proximity / emotional-load context the result.md author wants to surface during interpretation lives at the interpretation layer, not as a spec requirement. The §5.1 falsification bar is byte-identical with v2; the §5.0 headline cell is byte-identical with v2; the cut does not affect any test mechanic.
...
**Status: LOCKED 2026-06-17 by user acceptance at revision r2 (with the r2-pre-lock amendment + r2-post-amendment-cut applied).** Per [`hypothesis_lock_process.md` §3.4](../../../methodology/hypothesis_lock_process.md#34-audit-step-step-2-of-the-arc), the canonical audit step has completed (the [v3 audit report](../../../reviews/HA-C4b-v3-2026-06-16.md) verdict was PASS with caveats; all five Section-4 items closed in r2 above as wording-tightenings + locked descriptive context which was then partially CUT per user option B). Per [`hypothesis_lock_process.md` §3.6](../../../methodology/hypothesis_lock_process.md#36-re-audit-step-stage-4-of-the-arc), re-audit is compressed for r2 because the changes are mechanical (wording-tightenings, sentence additions, and scope reduction via the §8.x cut) with no architectural change, no new statistical machinery, and no falsification-bar change — the audit's own Section-4 items 1-4 were all labelled wording-tightenings that close without a v4; item 5's lock-time date elicitation was absorbed then cut per user direction. Compression justification per §3.6: *"r2 changes are wording-tightenings absorbing audit Section-4 items 1-4 + a §8.x scope reduction per user option B; no architectural change; no falsification-bar change; the audit verdict explicitly states the items close without a v4."*

The four [§3.8 lock-blocking gates](../../../methodology/hypothesis_lock_process.md#38-lock-step-stage-5-of-the-arc) confirmed at the lock commit:
...
- **Pacing-behaviour confounder (qualitative-only at v3)**. The §4.2 exertion-conditioning column `exertion_class_lagged_lcera` captures **physical exertion only** (Garmin-derived from heart-rate / motion / training-load aggregates). It does NOT capture:
  ...
  **The result.md must surface this caveat with the verdict**. The pacing-behaviour confounder reading is qualitative-only and applies to every verdict branch (see §9). v3 does NOT lock concrete date anchors in the spec record; if the user wants to attach pacing-era / event-proximity descriptive context during result.md interpretation, that is post-test commentary, not a spec-locked artefact (per user direction: "this is data interpretation, run the test first").

  **Note on existing data**: the corpus has partial quantitative proxies for emotional / cognitive load (`cat_belasting_emotioneel`, `cat_belasting_cognitief`, `state_symptoom_emotioneel`, `state_symptoom_cognitief`, `in_pwc_reintegratie_2023`; see [`DATA_DICTIONARY.md` §9, §11](../../../DATA_DICTIONARY.md)). v3 does NOT use these in §4.2 conditioning (would be a §3.7 approach change requiring new audit gate; queued as a methodology MD). They are available to the result.md author as interpretive context if needed; they are NOT spec-locked.

### 8.x ~~Locked pacing-era + emotional-event date anchors~~ — REMOVED per user direction 2026-06-17

Originally added in r2 to absorb audit Section-4 item-5 (lock-time date elicitation). User directed 2026-06-17 to cut this sub-block (option B in the discipline-cost-vs-spec-weight trade): the date anchors were not doing test-statistical work, only descriptive-annotation work, and that annotation can live as result.md commentary at interpretation time rather than as a spec-locked artefact. The qualitative pacing-behaviour caveat above stays; the concrete dates are NOT spec-locked. If the result.md author wants to attach era-classification or event-proximity flags during interpretation, that is post-test descriptive commentary per §3.7 and does not modify any §5 verdict.

The audit's L4.7 + Section-4 item-5 concerns about post-result interpretive flexibility are partially repriced as a result: the v3 spec accepts the flexibility cost in exchange for a slimmer spec. The §8 qualitative caveat + §9 verdict-invariant treatment remain the only discipline against post-result fitting; the date anchors no longer help.

(The original §8.x date-anchor content is preserved in the git history at commit `7595a8b` / `a7bf32a` for audit traceability; if a future v4 needs to re-introduce lock-time date elicitation, those commits are the reference.)
...
**Verdict-invariant pacing-behaviour reading** (r2, post-cut: applies qualitatively to every verdict branch per audit Layer 4.7 + Section-4 item-4). The pacing-behaviour confounder per §8 is a property of how the §4.2 exertion-conditioning interacts with the corpus's mix of crash mechanisms, NOT of the verdict label. Every result.md §9 branch must restate the §8 caveat: the v3 verdict speaks only to the **physically-exertion-conditioned subset of crashes** and does not generalise to emotionally / cognitively triggered crashes.

**Each verdict branch's interpretation** (the substantive readings are verdict-conditional; the §8 caveat is invariant):

- **Pooled-unmedicated SUPPORTED** (v3): the Wiggers C4 motion-filtered stress-elevated-minute count discriminates pre-crash from null windows on the unmedicated phase POOLED ARM, **under the v3 1b.ii-dropped day-validity discipline**, **for the §4.2 physically-exertion-conditioned subset of crashes only**. The verdict does NOT generalise to emotionally / cognitively triggered crashes (the §4.2 gate excluded them). The §4.3 1b.ii-override discipline cost (audit Layer 2.5 substantive concern; §8 v3 caveat) is restated: the v3 SUPPORTED reading is a §3.3 same-session-refinement-discipline verdict, not a strict-pre-registration verdict; a v4 fresh-session redraft of the §10.2 fix space (with 2023-02-04's identity held out) would be the strictest-discipline replication.
...
- **Pooled-unmedicated INCONCLUSIVE** (v3): n drops below 10 at the final exclusion. v3 §10.2 makes the dry-run gate symmetric with the full-run gate (both apply gates 1 + 1b.i only, no 1b.ii); the v2-style asymmetry-driven INCONCLUSIVE is structurally impossible in v3. **If INCONCLUSIVE fires in v3, the cause is §4.5 baseline-availability exclusion (not the v2 1b.ii-vs-1b.i asymmetry that v3 closed; not §4.3 1b.ii which v3 dropped).** Per v3 §9 INCONCLUSIVE branch: halt + recommend v4 only if a recoverable operationalisation refinement exists; otherwise descriptive companions become the only output and the pacing-behaviour caveat is restated. (Side-observation closure from audit: v3 has *one fewer trigger* for INCONCLUSIVE than v2 — 1b.ii-dropping-n is no longer in scope.)
...
- **Train-only vs validate-only directional inconsistency within unmedicated**: v2's test observed train (a) = 42.9% / validate (a) = 0%. v3 with 2023-02-04 restored may shift train (a) higher; validate (a) is structurally low at n = 2. The pooled verdict stands as the locked single-cell verdict; directional inconsistency is reported descriptively per v2 §9. Do NOT retroactively re-split the headline. Any per-era / per-pacing-context interpretation lives at the result.md author's interpretation layer, not in the spec.
...
- The §8 pacing-behaviour confounder caveat (qualitative) must be surfaced with the verdict block. The qualitative caveat applies to every §9 branch.
- ~~§8.x annotation table~~ — CUT per v3 r2-post-amendment-cut 2026-06-17. No spec-locked per-episode annotation table is required. The result.md inherits v2's per-episode table layout (date, era, max_signed_z, triggered yes/no); any pacing-behaviour / emotional-load / event-proximity context the result.md author wants to surface lives at the interpretation layer, not as a spec requirement.
