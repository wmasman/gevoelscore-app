...
**Pre-registration (current)**: [`hypothesis.md`](hypothesis.md) — v3 **r2 LOCKED-PENDING-USER-ACCEPTANCE 2026-06-16**. Fresh-session `/research-review` audit completed ([report](../../../reviews/HA-C4b-v3-2026-06-16.md), verdict PASS-with-caveats); all five Section-4 items closed in r2 as wording-tightenings + locked descriptive-context (per [`hypothesis_lock_process.md` §3.5](../../../methodology/hypothesis_lock_process.md#35-revise-step-stage-3-of-the-arc-r2--the-bulk-of-methodological-strengthening) + [§3.6 compression](../../../methodology/hypothesis_lock_process.md#36-re-audit-step-stage-4-of-the-arc)). r2 absorbed: empirical 2023-02-04 coverage check (confirmed pathological pattern Q3=8; §4.3 rationale rewritten as honest override-as-trade-off); audit Layer 2.5 substantive concern named in §8; §3.3 discipline-stretch acknowledgment in Authorship; verdict-invariant pacing-era annotation per §9; §8.x locked descriptive-context sub-block with concrete pacing-era + PWC-flag + tweede-spoor date anchors derived from existing master-CSV columns; cat_belasting_emotioneel / cognitief proxy reality reframed (the queued-primitive line was wrong; the proxy exists with sparsity caveats). **Headline cell UNCHANGED** from v2 (`unmedicated × pooled × S60_Mlow × N_std=1.5 × primary 4d × one-sided elevated`); §5.1 byte-identical with v2. Lock signal awaits explicit user acceptance + commit message naming the four §3.8 gate confirmations.
