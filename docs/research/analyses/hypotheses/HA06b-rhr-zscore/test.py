"""HA06b - Z-score bidirectional RHR delta as crash precursor.

Implements the protocol pre-registered in hypothesis.md exactly.

Same data sources + windowing as HA06. Only substantive difference:
threshold is N_std (multiples of baseline std) instead of N bpm
(absolute).
...
# Pre-registered SUPPORTED bar - same as HA06
CRIT_A_FRAC = 0.60
CRIT_B_DISC_PP = 15
...
def compute_baseline_mu_sigma(
    ...
    """Trimmed-mean + std of RHR over [d-90, d-30]. Returns (mu, sigma)
    or (None, None) if insufficient prior days or sigma below floor.
    """
    ...
    if sigma < MIN_BASELINE_STD:
        return None, None  # flagged low-variability; skip
    return mu, sigma
...
def per_day_z(
    ...
    """Returns (signed_z, abs_z) for date d. None if RHR or (mu, sigma) undefined."""
    if d not in rhr_by_date:
        return None, None
    mu, sigma = compute_baseline_mu_sigma(d, rhr_by_date)
    if mu is None or sigma is None:
        return None, None
    signed = (float(rhr_by_date[d]) - mu) / sigma
...
def episode_profile(
    ...
    for i in range(1, leadup_days + 1):
        ...
        signed, _ = per_day_z(wd, rhr_by_date)
        if signed is None:
            continue
...
def dry_run(rhr_by_date: dict[date, int], crashes: list[date], k: int = 3) -> None:
    ...
    for ref in sample:
        ...
        print(f"  {'date':12} {'rhr':>5} {'mu':>6} {'sigma':>6} "
              f"{'z':>7} {'|z|':>6}")
        for i in range(1, LEADUP_SECONDARY + 1):
            wd = ref - timedelta(days=i)
            n_total_lookups += 1
            rhr_val = rhr_by_date.get(wd)
            mu, sigma = compute_baseline_mu_sigma(wd, rhr_by_date)
            if rhr_val is None:
            ...
            if mu is None or sigma is None:
                n_low_var += 1
                print(f"  {wd.isoformat():12} {rhr_val:>5} {'LOW':>6} {'VAR':>6} "
                      f"{'N/A':>7} {'N/A':>6}")
                continue
            signed = (rhr_val - mu) / sigma
            print(f"  {wd.isoformat():12} {rhr_val:>5} {mu:>6.2f} {sigma:>6.3f} "
...
def main() -> int:
    ...
    train_4d = collect_events(train_crashes, rhr_by_date, LEADUP_PRIMARY, MIN_LEADUP_VALID_PRIMARY)
    validate_4d = collect_events(validate_crashes, rhr_by_date, LEADUP_PRIMARY, MIN_LEADUP_VALID_PRIMARY)
    train_5d = collect_events(train_crashes, rhr_by_date, LEADUP_SECONDARY, MIN_LEADUP_VALID_SECONDARY)
    validate_5d = collect_events(validate_crashes, rhr_by_date, LEADUP_SECONDARY, MIN_LEADUP_VALID_SECONDARY)
    ...
    print(f"  {'arm':45} {'verdict':>11} {'frac_event':>11} {'frac_null':>11} "
        ...
        for N_std in [N_STD_PRIMARY, N_STD_SECONDARY, N_STD_SENSITIVITY]:
            for direction_label, bidirectional in [("bidirectional", True), ("one_sided", False)]:
                for era_label, events in [("train", train_evt), ("validate", validate_evt)]:
                    ...
                    r = evaluate_arm(events, null_sample, N_std, bidirectional)
                    results[key] = r
                    verdict = r["verdict"]
                    if "frac_event" in r:
                        print(f"  {key:45} {verdict:>11} "
    ...
    one_sided_train = results["4d_primary_Nstd1.5_one_sided_train"]
    one_sided_val = results["4d_primary_Nstd1.5_one_sided_validate"]
