...
## Cross-test note from HA-P6 v3 (added 2026-06-17, reviewer-mode-with-authorization per CONVENTIONS §1.2)

*Per HA-P6 v3 §9 first-branch propagation #3 ("Inform HA-P7's window-length assumption"). Source: [`../HA-P6/result.md`](../HA-P6/result.md) at commit `19d33e4` (collision-commit; HA-P6 files attributable to v3) + audit-trail commit `a980b1c` + reader's notes commit `bbcb478`. Interpretive overlay on the HA-P7 NOT-SUPPORTED verdict above; NOT a post-hoc modification of the HA-P7 spec.*

**HA-P6 v3 finding relevant to HA-P7's 14d-window assumption.** HA-P6 v3 test-executed 2026-06-17 finds §9 first-branch FIRES: 4 / 7 channels statistically-distinguishable from Arm-A matched-deep-trough control (`stress_mean_sleep`, `all_day_stress_avg`, `bb_lowest`, `stress_low_motion_min_count_S60_Mlow`). Median recovery-completion-day across these 4 channels = **2.5 days**. Per HA-P6 v3 §9 first-branch downstream propagation: *"if the median recovery-completion-day-estimate across channels falls within [3, 5] days, P7's 14d window is a generic period covering recovery + further; if it extends to 7-10+ days, P7's 14d window is recovery-specific."* The observed 2.5 days is *below* the lower bound of the spec's "[3, 5] → generic" range — **the 14d window is even more generic than the spec's generic-period branch anticipated**: it covers ~2.5 days of recovery + ~11.5 days of post-recovery activity.

**Implication for the HA-P7 NOT-SUPPORTED verdict.** HA-P7's verdict reads NOT-SUPPORTED — `crash_count_14d` does NOT predict elevated next-crash risk on the locked headline cell. HA-P6 v3's finding adds nuance: there IS a real post-crash recovery signature (4 / 7 channels distinguishable), but it concentrates in the first ~2-3 days post-episode-end, not throughout the 14d window. **The HA-P7 NOT-SUPPORTED verdict therefore does NOT refute the "post-crash window is distinctive" claim — it refutes the narrower claim that this distinctiveness aggregates into elevated next-crash risk at the 14d window scale.** Three readings of the joint result:

1. **Window-scale mismatch**: the post-crash distinctiveness exists at a narrower timescale (~2-3 days) than HA-P7's 14d window. A hypothetical HA-P7-companion test at `crash_count_3d` or `crash_count_5d` would be a direct test of whether the post-crash distinctiveness translates into elevated next-crash risk at the recovery-period timescale.
2. **No translation from recovery shape to risk elevation**: the distinctive recovery signature exists (HA-P6 v3) but does not actually translate into elevated next-crash probability at any window scale — the autonomic recovery shape and the next-crash-risk signal are decoupled. HA-P7's NOT-SUPPORTED then stands as a meaningful negative on the recovery-debt mechanism reading.
3. **n=29 sparsity ceiling**: a moderate effect at the recovery-period timescale exists but is below the n=29 detection floor at both HA-P6 (descriptive distinguishability) and HA-P7 (predictive elevation). Both tests under-power on the relevant timescale; both verdicts honest given the corpus.

**HA-P7 caveat 1 disambiguation update**: caveat 1 above ("Causal-attribution ambiguity is irreducible") names the recovery-debt-vs-shared-cause split. HA-P6 v3's evidence supports recovery debt at the within-episode-tail timescale (rate↔duration correlations CI-excludes-0 on `all_day_stress_avg` ρ=+0.285 [+0.021, +0.585] and `stress_low_motion_min_count_S60_Mlow` ρ=+0.389 [+0.013, +0.709] per HA-P6 v3 §4.8.4) AND argues against recovery debt at the across-episode timescale (`all_day_stress_avg` completeness↔next-interval ρ=-0.338 [-0.493, -0.168] is INVERSE direction from naive recovery debt per HA-P6 v3 reader's notes Note 2). The HA-P7 NOT-SUPPORTED verdict reads consistently with the across-episode-recovery-debt-NOT-supported finding from HA-P6 v3. For downstream consumers: the recovery-debt mechanism reading is partly empirically settled — within-episode-tail YES, across-episode NO — without HA-P7 itself needing to re-run.

**§4.5.4 covariate-sensitivity follow-up suggestion (per HA-P6 v3 §9 bullet 7).** HA-P6 v3 §4.8.4 finds CI-excludes-0 on `all_day_stress_avg` rate↔duration and `stress_low_motion_min_count_S60_Mlow` rate↔duration. Per HA-P6 v3 §9 bullet 7 propagation these are candidate non-`crash_count_14d` covariates for HA-P7's §4.5.4 covariate-sensitivity arm. A future HA-P7-v2 (if triggered for unrelated reasons) could add these as covariate-adjusted predictors; this is a queued-for-future-test note, not an action against the current HA-P7 v1 LOCKED spec.

---

*Result emitted by `test.py` on the locked pre-registration. Raw result data (bootstrap CIs, beta arrays, per-phase cells) in `result-data.json` alongside this file. Cross-test note above added 2026-06-17 per HA-P6 v3 §9 first-branch propagations #3 + #7; interpretive overlay per CONVENTIONS §1.2, NOT a post-hoc spec modification. Any post-result modification of the spec creates HA-P7-v2 with this v1 archived.*
