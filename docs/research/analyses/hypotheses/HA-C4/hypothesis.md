...
**Status: LOCKED 2026-06-18 by user acceptance at revision r2** by Claude (Opus 4.7) in reviewer-mode-with-authorization. The fresh-session §3.4 audit (verdict PASS-with-caveats; [report](../../../reviews/HA-C4-v2-2026-06-18.md)) was absorbed in r2 above as four mechanical wording closures of the audit's Section 4 side observations. **L2.5 disposition** (audit-binding lock-stage call): user accepted at lock time that the v1 r2 "priced-in" disposition of the §3.2-clause shared-context drafting concern **carries forward to v2** — v2's structural inheritance from v1's pre-dry-run verdict bands (0.5 contribution + ≥ 2.0 SUPPORTED bar byte-identical with v1's pre-halt lock) materially weakens the calibration-bias mechanical link of the L2.5 fire. **§3.6 re-audit compressed** per the criteria (r2 changes mechanical — wording-tightenings + construction-rule clarification + reporting-discipline annotation + pre-lock data-presence verification; no architectural change, no new statistical machinery, no falsification-bar change).
...
1. **Power-calc dispatch — MET**. §8 carries the Daza 2018 within-subject design dispatch verbatim (inherited from v1). The block-permutation null at E[L]=7 (§4.9) is the within-subject inferential machinery; the §5.1 (a) discrimination + (b) effect-size gates determine per-channel SUPPORTED/REFUTED rather than asymptotic-power thresholds; INCONCLUSIVE cells are sample-size shortfalls handled by §5.3 + §5.4.
2. **Multi-comparison discipline — MET**. §5.0 + §5.3 + §5.5 lock the 3-channel triad with the INCONCLUSIVE-aware verdict bands (3.0 strong / 2.0-2.5 SUPPORTED / 1.0-1.5 PARTIAL / <1.0 REJECTED) as the project-canonical multi-channel discipline. Holm step-down across channels per era is reported as a fragility flag (with the r2 fewer-comparisons annotation requirement when an INCONCLUSIVE cell is present).
3. **Register-row pointer — MET at this lock-commit**. [`wiggers_testable_hypotheses.md` C4 row](../../../wiggers_testable_hypotheses.md) updated at lock-commit time to point at this v2 spec (was previously pointing at v1 at the v1 r2 lock-commit).
4. **Re-audit clean OR §3.6 compression — MET via compression**. The §3.4 audit was clean on three Layer-2 verification asks + L1 + L3 + most of L4; one L4.5 minor inherited carry-forward; one L2.5 substantive concern resolved by user acceptance at lock time (priced-in carries forward). The four Section-4 side observations all closed in r2 as mechanical fixes; the audit verdict explicitly states the structural deltas are *"each structurally honest and well-bounded"* and PASS-with-caveats.
