...
The audit's strengthening recommendations #5 (per-phase minimum-n gate on §9 third-branch) and #6 (register-row pointer at lock) are NOT addressed in this r2 — recommendation #5 is queued for r3 / v2 (needs a defined threshold + propagation logic); recommendation #6 is executed at lock per §3.8 gate 3 in the r3 entry below.

**Revision 2026-06-15-r3** (same session, lock signal). No content changes from r2; this entry records explicit user acceptance under option-A compression per [`hypothesis_lock_process.md` §3.6](../../../methodology/hypothesis_lock_process.md#36-re-audit-step-stage-4-of-the-arc). The audit's recommendation #6 (register-row pointer) is executed in the same commit as this r3 lock: [`personal_hypotheses.md` P6 entry](../../../personal_hypotheses.md) receives the top-level supersession blockquote pointing to this pre-reg per §3.8 gate 3.

**Status: LOCKED 2026-06-15 by user acceptance under option-A compression.** The pre-registration is locked at the state of revision r2 (this file's HEAD). **Compression**: re-audit skipped per [§3.6 acceptability criteria](../../../methodology/hypothesis_lock_process.md#36-re-audit-step-stage-4-of-the-arc) — r2 closures were mechanical (power-calc dispatch added; algorithmic enumeration pre-specs filled into §4.8.3; §9 trigger-phrase binding; single-cell-lock per established HA-C4b r2 + HA-P7 r2 pattern), no architectural change; pre-reg is Layer 1 descriptive characterisation per [CONVENTIONS §2.1](../../../CONVENTIONS.md#21-descriptive-before-inference) with no SUPPORTED bar (§5 is "findings shape", not a falsification criterion). Further modifications create HA-P6-v2 with v1 archived. The next session writes `script.py` + runs + emits `result.md` per §10. After lock, [`/research-review`](../../../reviews/README.md) must run in a fresh session (no shared drafting-session context); the review report lands in [`reviews/`](../../../reviews/) with the addendum *"Fresh session — no exposure to the drafting context; doc-only knowledge."*
