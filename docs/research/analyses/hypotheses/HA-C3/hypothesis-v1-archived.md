...
**Revision 2026-06-23-r2** (post-audit, shared-context with drafting per [`hypothesis_lock_process.md` §3.5](../../../methodology/hypothesis_lock_process.md#35-revise-step-stage-3-of-the-arc-r2--the-bulk-of-methodological-strengthening) + §3.6 compression). Six spec amendments absorbed from the [fresh-session `/research-review` audit report](../../../reviews/HA-C3-2026-06-22.md) (verdict: **PASS with caveats**; commit `8f3f269`). The audit found: no Layer-1 fires, no Layer-2 fires, one Layer-3 substantive (L3.4 — §4.5.1(c) spline F-test silent on permutation null) + one Layer-3 minor (L3.3 — 3-condition gate PARTIAL-band multi-comparison disclosure; not closed at r2 per the handoff's strict 6-item list and not promotion-affecting), no Layer-4 substantive, plus one arithmetic-error side observation (§4.5.1(b) companion contrast orthogonality claim wrong) and four drafter open questions (Q1 spline midpoints; Q2 §7.3 halt-option default; Q3 within-consolidation aggregation; Q4 E[L]\* second derivation). Six closures absorbed mechanically — no architectural change, no falsification-bar change, no new statistical machinery:

- **§4.5.1(c) spline F-statistic bound onto §4.7 block-permutation E[L]=7** (closes L3.4 substantive). The parametric F p-value is anti-conservative on autocorrelated `gevoelscore` residuals per [`permutation_null_block_length.md`](../../../methodology/permutation_null_block_length.md) — the binding brings condition (c) onto the same null-machinery foundation as conditions (a) Jonckheere-Terpstra and (b) second-difference contrast. Parametric F retained as descriptive-only alongside.
- **§4.5.1(b) companion contrast vector replaced with the textbook orthogonal quadratic `(+2, -1, -2, -1, +2)`** (closes arithmetic side observation per **Option A**). The original `(+1, +2, 0, -2, -1)` has dot product −8 with the linear contrast `(-2, -1, 0, +1, +2)`, so was NOT orthogonal as claimed. The replacement has dot product 0 (verified inline at §4.5.1(b)), sums to zero, and direction (`c · m < 0` under SUPPORTED) is consistent with the §4.5.1(b) primary `S < 0` discipline. §4.5.2 secondary descriptive outcomes updated to match.
- **§4.5.1(c) B1-midpoint dropped from spline-second-derivative count** (closes drafter Q1). The natural-cubic boundary condition forces near-zero second derivative throughout the leftmost segment (x ≤ 20); B1's midpoint at x=10 carries no convexity information by construction. Gating becomes ≥ 3 of 4 midpoints from segments {[20,30), [30,40), [40,60), [60,100]} (not ≥ 3 of 5). §5.1 verdict-bar wording updated to match.
- **§7.3 halt-option-A pre-committed as the lock-time default** (closes drafter Q2). Halt-option-B requires explicit user override before v2 redraft. Removes one round-trip from the v2 cycle if §7.5 gate 1 fires; B5 underpower is the expected fragility per §7.2 forecast.
- **§4.4 within-consolidation §5.A recursive 4-bin reduction** (closes drafter Q3). The §7.3 halt-option-A discipline applies recursively to the consolidation secondary: if 4 of 5 consolidation bins have ≥ 30 but B5 has < 30, report on the 4-bin reduction; below 4 bins with ≥ 30, suppress the consolidation secondary entirely.
- **§4.7 E[L]\* second derivation on the bin-label sequence** (closes drafter Q4). Two `E[L]*` values reported: (i) on the linear-residual series (continuous-predictor autocorrelation; standard default) and (ii) on the bin-label categorical sequence (the actual permutation target's autocorrelation). Both fire the factor-of-2 flag if `|E[L]* - 7| / 7 > 0.5`.

**Status: r2 LOCKED 2026-06-23 by user acceptance** by Claude (Opus 4.7) in producer-mode under user authorisation. The fresh-session [§3.4 audit](../../../reviews/HA-C3-2026-06-22.md) (verdict PASS-with-caveats; commit `8f3f269`) was absorbed in r2 above as six mechanical closures of the audit's L3 substantive fire + arithmetic side observation + four drafter open questions. **§3.6 re-audit compressed** per the criteria (r2 changes mechanical — block-permutation binding rebinds an existing §4.7 mechanism onto condition (c) without inventing new machinery; orthogonal-contrast swap is an arithmetic cleanup that preserves the companion's reporting-only role; the four Q-closures are wording-tightenings of structurally pre-spec'd handling; no architectural change, no new statistical machinery, no falsification-bar change).
...
1. **Power-calc dispatch — MET**. §8 caveat 1 carries the [Daza 2018](../../../literature/methodology/daza_2018_self_tracked_n_of_1_counterfactual.pdf) within-subject design dispatch verbatim (`power calc inapplicable per Daza 2018 within-subject design`). The block-permutation null at E[L]=7 (§4.7, now binding all three primary conditions per r2) is the within-subject inferential machinery; the §5.1 3-condition gated verdict determines SUPPORTED/PARTIAL/REJECTED rather than asymptotic-power thresholds; INCONCLUSIVE per §5.2 is the operational definition of "underpowered for this cell".
2. **Multi-comparison discipline — MET**. §5.0 single-cell headline lock binds one headline cell (unmedicated × 5-bin × `gevoelscore` × 3-condition gated outcome); every other arm — secondary phases (§4.4), dose-adjusted cross-phase (§4.8), crash-drop (§4.6), t+1 lagged (§4.8), train+validate M3 overlay (§4.8) — is descriptive sensitivity only and cannot promote to SUPPORTED. Within the headline cell, the conjunctive 3-of-3 SUPPORTED gate provides effective multiplicity control at the SUPPORTED bar (PARTIAL band's nominal α is not Bonferroni-corrected because the three tests share substantial structural dependence on the same bin-means; per the audit's L3.3 minor, PARTIAL is a descriptive band rather than an inferential-band claim); §4.5.2 pairwise Mann-Whitney across 4 adjacent bins uses Holm step-down (cutoffs α/4, α/3, α/2, α/1 at α = 0.05).
3. **Register-row pointer — MET at this lock-commit**. [`wiggers_testable_hypotheses.md` Tier 1 C3 row](../../../wiggers_testable_hypotheses.md#tier-1--source-verified-verbatim--no-family-history-priority-pre-regs) updated at lock-commit time to point at this r2 LOCKED spec (was previously pointing at r1 drafted at commit `ec99c2f`).
4. **Re-audit clean OR §3.6 compression — MET via compression**. The §3.4 audit was clean on L1, L2, most of L3 (one substantive fire on (c), one minor on PARTIAL band) and most of L4 (one minor dispatched via §8 caveat 8 spike-companion). The L3.4 substantive fire is mechanically closed by r2's §4.5.1(c) binding onto the existing §4.7 machinery (no new statistical machinery invented). The arithmetic side observation + four drafter open questions are mechanical closures. The audit verdict explicitly states the L3.4 fire is "a single targeted fix that closes by binding (c) onto the §4.7 block-permutation machinery already specified for (a) and (b)" — i.e. the audit pre-authorised the compression path.

| revision | date | summary |
|---|---|---|
| r1 | 2026-06-22 | drafted at commit `ec99c2f`; fresh-session-isolated drafter; bin-spec/contrast/spline/seed/citalopram-approach all pre-committed before joint-distribution inspection. |
| r2 | 2026-06-23 | §3.6-compression absorb of audit fires per `8f3f269` report: §4.5.1(c) spline F bound onto §4.7 block-permutation (closes L3.4 substantive); §4.5.1(b) companion contrast vector replaced with textbook orthogonal quadratic `(+2, -1, -2, -1, +2)` per Option A (closes arithmetic error); four drafter open questions addressed: B1 midpoint dropped from spline-sign count (Q1) + §7.3 halt-option-A pre-committed (Q2) + within-consolidation recursive 4-bin reduction (Q3) + bin-label E[L]\* second derivation (Q4). LOCKED 2026-06-23. |
...
- **§5.A within-consolidation replication**: if consolidation phase (~638 days, of which ~600 have gevoelscore at the §4.3 gate) has ≥ 30 observations in each of the 5 bins, replicate the convexity test within consolidation. **Within-consolidation §5.A replication applies the same §7.3 halt-option-A 4-bin reduction recursively if any within-consolidation bin has n < 30** (specifically: if 4 of 5 bins have ≥ 30 but B5 has < 30, report the consolidation replication on the 4-bin reduction with B4 widened to absorb B5, mirroring the unmedicated §7.3 halt-option-A discipline; if fewer than 4 bins have ≥ 30, suppress the consolidation secondary entirely). **Reported as descriptive context only**: a SUPPORTED-within-unmedicated AND SUPPORTED-within-consolidation reading is "the convexity shape is structural-within-subject across both phases"; a SUPPORTED-within-unmedicated AND REFUTED-within-consolidation reading is "the convexity shape is medication-state-dependent" (substantively interesting but does NOT modify the primary verdict).
...
**Companion contrast vector for transparency**: report `c · m` where `c = (+2, -1, -2, -1, +2)` (the textbook orthogonal quadratic contrast for 5 evenly-spaced points). Verification: sum = 2 + (-1) + (-2) + (-1) + 2 = 0 ✓; dot product with the linear contrast `(-2, -1, 0, +1, +2)` = 2·(-2) + (-1)·(-1) + (-2)·0 + (-1)·1 + 2·2 = -4 + 1 + 0 - 1 + 4 = 0 ✓ (orthogonal). Direction: under SUPPORTED (concave-down / accelerating-decrement shape, where m2, m3, m4 lie above the linear chord from m1 to m5), the middle weights (-1, -2, -1) act on the elevated middle bin-means and the endpoint weights (+2, +2) act on the high-low extremes, yielding `c · m < 0` — consistent with the §4.5.1(b) primary statistic's `S < 0` SUPPORTED direction. Not part of the §5 verdict bar; reported in result.md.

**Condition (c) — Spline non-linearity test**: natural cubic spline regression of `gevoelscore = f(all_day_stress_avg)` with **4 internal knots placed at the bin boundaries (20, 30, 40, 60)**. Compare the full-spline model against the linear-only model via the F-statistic on the difference in residual sum of squares (degrees of freedom = number of non-linear basis terms = 3 for a natural cubic spline with 4 internal knots). **F-statistic significance is computed via block-permutation at E[L]=7 per §4.7** (same machinery as condition (a) Jonckheere-Terpstra and condition (b) second-difference contrast): compute the F-statistic on the observed bin-label sequence, then on each of B = 10,000 null draws under the same §4.7 stationary-bootstrap label-resampling target, and report the empirical one-sided p-value. The parametric F-distribution p-value is reported alongside as descriptive only (anti-conservative on the autocorrelated `gevoelscore` residuals per [`permutation_null_block_length.md`](../../../methodology/permutation_null_block_length.md) discipline). **Pass condition**: empirical block-permutation p < 0.05 AND visual inspection of the spline confirms convexity (concave-down with negative slope, equivalently: monotone-decreasing with negative second derivative across the support). Visual gating is operationalised via the **sign of the spline's second derivative at the bin midpoints**: pass requires negative second derivative at ≥ 3 of 4 bin midpoints from segments {[20,30), [30,40), [40,60), [60,100]} (B1's midpoint at x=10 is dropped from the count because the natural-cubic boundary condition at x=20 forces near-zero second derivative throughout the leftmost segment by construction; sign at x=10 carries no convexity information). Reported as a numerical check, not subjective visual approval.
...
- **Companion contrast** `c · m` value with `c = (+2, -1, -2, -1, +2)` (textbook orthogonal quadratic for 5 evenly-spaced points) per §4.5.1 condition (b) note.
...
**E[L]\* companion + factor-of-2 flag**: per the factor-of-2 flag pattern, report **two** `E[L]*` values: (i) the standard derivation on the residuals from the linear `gevoelscore ~ all_day_stress_avg` fit (post-Z-score-of-residual ACF) — measures autocorrelation in the continuous-predictor read; and (ii) a second derivation on the bin-label sequence itself — measures autocorrelation in the categorical version actually used as the resampling target (the bin labels inherit `all_day_stress_avg` autocorrelation, and the bin-label series is the actual permutation target so its `E[L]*` is the structurally faithful complement to (i)). Each fires the flag if `|E[L]* - 7| / 7 > 0.5`. Per the methodology MD, the flags fire only on SUPPORTED verdicts; for PARTIAL or REJECTED, descriptive context only.
...
A condition is **MET** when its test statistic passes the §4.5.1 pass condition (one-sided empirical p < 0.05 AND correct direction; for condition (c) additionally the spline-second-derivative sign check at ≥ 3 of 4 bin midpoints from segments {[20,30), [30,40), [40,60), [60,100]} per §4.5.1(c) — B1's midpoint at x=10 is dropped because the natural-cubic boundary condition forces near-zero second derivative there by construction).
...
- **Halt option A** (pre-committed default at r2 lock per the convexity claim's structure): redraft as HA-C3-v2 with B4 widened to absorb B5 (e.g. `[40, 100]` becomes the single high-stress bin); the convexity test reduces to 4 bins with 2 second-differences. The cost is the loss of the high-stress-tail-most-extreme reading where the convex cost is sharpest — explicit trade-off documented and accepted at lock time.
- **Halt option B** (explicit user override only): continue with B5 INCONCLUSIVE per §5.2; the 3-condition test runs on the 4-bin reduction; report B5 descriptively. **NOT preferred**: drops the load-bearing high-tail reading.

**Pre-commit at r2 lock**: if §7.5 gate 1 fires, the v2 redraft adopts halt option A by default; halt option B requires explicit user invocation before the v2 redraft. This removes one round-trip from the v2 cycle if the gate fires; the pre-commitment is methodologically clean because B5 underpower is the *expected* fragility per §7.2's distribution forecast.
...
*Pre-registration drafted 2026-06-22 r1 by Claude (Opus 4.7) in producer-mode under user authorisation per [CONVENTIONS §1.1](../../../CONVENTIONS.md#11-producer-mode-claude-edits-the-docs--codebase-the-default) per the session handoff brief `session-HA-C3-pre-reg-drafting-handoff-2026-06-22.md`; **r2 LOCKED 2026-06-23 by user acceptance** per the session handoff brief `session-HA-C3-r2-handoff-2026-06-22.md` after the fresh-session [§3.4 audit](../../../reviews/HA-C3-2026-06-22.md) returned PASS-with-caveats (commit `8f3f269`). All four [§3.8 lock-blocking gates](../../../methodology/hypothesis_lock_process.md#38-lock-step-stage-5-of-the-arc) confirmed: power-calc dispatch + single-cell headline lock + register-row pointer updated at lock-commit + §3.6 compression in lieu of re-audit. **Test execution is a separate post-lock session per the canonical arc.**
