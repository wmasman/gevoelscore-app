...
### §3.8 gate-verification block (confirmed MET at v2 r2 LOCK per [`hypothesis_lock_process.md` §3.8](../../../methodology/hypothesis_lock_process.md#38-lock-step-stage-5-of-the-arc))

The four [§3.8 lock-blocking gates](../../../methodology/hypothesis_lock_process.md#38-lock-step-stage-5-of-the-arc) are confirmed MET at this v2 r2 lock-commit:

1. **Power-calc dispatch — MET** via §8 caveat 1 (Daza 2018 within-subject design citation; block-permutation null at E[L] = 7 named as the within-subject inferential machinery; the §5.1 3-condition gated verdict determines SUPPORTED/PARTIAL/REJECTED; INCONCLUSIVE per §5.2 is the operational definition of "underpowered for this cell").
2. **Multi-comparison discipline — MET** via §5.0 single-cell headline lock (one headline cell: unmedicated × 4-bin × `gevoelscore` × 3-condition gated outcome) + §4.6 Holm step-down across 3 adjacent-bin pairs (v2 4-bin scheme); every other arm is descriptive sensitivity only and cannot promote to SUPPORTED. The conjunctive 3-of-3 SUPPORTED gate provides effective multiplicity control at the SUPPORTED bar; PARTIAL is a descriptive band per the v1 r2 audit's L3.3 framing.
3. **Register-row pointer — MET at this lock-commit (DISPATCHER HANDLES)**: the register-row update for HA-C3 ALREADY happened at v1 r2 lock (`de22b68`); the row references file path `hypothesis.md` which is now v2 by inheritance. The v2 r2 register-row pointer update is **DEFERRED to the dispatcher** — Wave 5 r2 sessions (HA-C3 v2 r2 + HA-C3p r2 + HA-C4c r2) all touch the C3 register row; the dispatcher will do a single consolidated row update after all 3 r2 sessions land, to avoid cherry-pick conflicts (HA-C3p r2 is running in parallel and also touches the C3 row). Gate 3 is MET because the file-path reference is stable across v1 → v2; the LOCKED-at-commit pointer refresh is dispatcher-deferred per Wave 5 coordination.
4. **Re-audit clean OR §3.6 compression — MET via compression** per audit + Option γ user-selected. Fresh-session audit `docs/research/reviews/HA-C3-v2-2026-06-23.md` (commit `e2bca27`) returned PASS-with-caveats with 4 mechanical wording fires; all 4 absorbed at this v2 r2 commit; §3.6 compression eligibility confirmed by the audit verdict + the user-selected Option γ; no architectural change, no falsification-bar change, no new statistical machinery introduced by the r2 absorb.
...
| v2 r2 | 2026-06-23 | §3.6-compression absorb of fresh-session audit fires ([`reviews/HA-C3-v2-2026-06-23.md`](../../../reviews/HA-C3-v2-2026-06-23.md) commit `e2bca27`; verdict PASS-with-caveats): (1) L1 sister-test block (§8 caveat 12) extended to name HA-C3p as personal-baseline sister operationalisation; (2) L1 STROBE-Item-12 §1 forward-reference added to §8 caveat 7 (operationalised-measures source-file traceability); (3) L2 §11 drafting-context operational consequence note added to §8 caveat 11 (confirmatory question + exploratory operationalisation-choice distinction per CONVENTIONS §4.3); (4) L4 §7.4 caveat-class tightening — inverted-U/threshold pattern replaced with "any other non-monotone shape including but not limited to U-shaped, inverted-U, or step-function patterns" per CONVENTIONS §4.2 (audit's specific recommended fix). All four fires mechanical wording absorbs; no architectural change, no falsification-bar change, no new statistical machinery. **Register-row pointer update DEFERRED to dispatcher** — Wave 5 r2 sessions (HA-C3 v2 r2 + HA-C3p r2 + HA-C4c r2) all touch the C3 register row; dispatcher consolidates the row update in a single post-cherry-pick commit to avoid cherry-pick conflicts. **LOCKED 2026-06-23** by user acceptance per Option γ + §3.6 compression eligibility per audit. |

**Status**: r2 LOCKED 2026-06-23 by user acceptance + §3.6 compression of audit PASS-with-caveats.
...
**Operationalised measures** (forward-reference added in v2 r2 per STROBE-Item-12 inheritance): `all_day_stress_avg` source per [DATA_DICTIONARY](../../../DATA_DICTIONARY.md) §C; `gevoelscore` per §S; both with documented coverage; computation paths traceable to [pipeline](../../../pipeline/) per CONVENTIONS STROBE-Item-12 inheritance; see §8 caveat 7 for full source-file traceability inventory.
...
11. **Drafting context disclosure** (NEW in v2): this v2 r1 was drafted in a fresh worktree-isolated session 2026-06-23 per the handoff brief. The drafter HAS seen the v1 partial-pool descriptives (pool n = 581, stress median 34, gevoelscore median 4, populated-bins trajectory 3.958 → 4.265 → 3.860 from v1 result.md / dry-run-report.md), and the §4.1 v2 bin spec was chosen with knowledge of this distribution. The data-exposure level is honestly disclosed in the Authorship block's "Data exposure context" — the v2 bin scheme is NOT a fresh-from-blind redraft, it is the minimum-viable v1-HALT response with the v1 partial-pool trajectory treated as a caveat-class prior per caveat 9. Per [`hypothesis_lock_process §3.2`](../../../methodology/hypothesis_lock_process.md#32-drafting-step-step-1-of-the-arc) drafting permitted; the §3.4 audit step is a separate fresh-session pass that should verify the v1 → v2 cascade is complete and that the data-exposure-conditional drafting did not import any v2-specific substantive claim. **Operational consequence (added in v2 r2)**: per [CONVENTIONS §4.3](../../../CONVENTIONS.md#43-confirmatory-vs-exploratory-distinction): the substantive Wiggers C3 convex-shape question stays **confirmatory** per the source-verified prior (Wiggers PDF 1357-1368); the v2 **operationalisation choice** (4-bin scheme collapsing v1's empty B1 into the new low-stress bin) is **exploratory-with-caveat** per the corpus-conditional redraft trigger (v1 r2 dry-run HALT on B1 [0,20) n=0 corpus-property finding). The distinction matters for §5 verdict interpretation: a SUPPORTED verdict supports the confirmatory question, while a PARTIAL or REJECTED verdict carries the additional operationalisation-choice uncertainty.

12. **Sister-test cross-references** (informational; inherited from v1, extended in v2 r2): HA-C4 v2 REJECTED at daily-aggregate; HA11 SUPPORTED on train; HA-C3 v2's primary cell (cross-day-aggregate shape) is structurally distinct from both. No prior import. **Sister pre-reg [HA-C3p](../HA-C3p/hypothesis.md)** is the personal-baseline operationalisation per [CONVENTIONS §3.1](../../../CONVENTIONS.md#31-personal-baseline-not-population-norm) (equal-N quintile bins on the participant's `all_day_stress_avg` distribution); HA-C3p tests the underlying convex-shape claim on the participant's actual stress range, while HA-C3 v2 tests Wiggers' verbatim 30→40 numerical anchor. The two are sister operationalisations of the Wiggers C3 substantive question; the 4-cell agreement matrix lives in HA-C3p result.md §6 open-questions when both tests run. HA-C3p's [README line 48](../HA-C3p/README.md) cross-references HA-C3 v2; this reciprocal pointer closes the asymmetry.
...
*Pre-registration drafted 2026-06-23 v2 r1 by Claude (Opus 4.7, 1M context) in producer-mode under user authorisation per [CONVENTIONS §1.1](../../../CONVENTIONS.md#11-producer-mode-claude-edits-the-docs--codebase-the-default) per the session handoff brief `session-HA-C3-v2-redraft-handoff-2026-06-23.md`. v2 r2 absorbed audit fires + LOCKED 2026-06-23 via §3.6 compression per the session handoff brief `session-HA-C3-v2-r2-handoff-2026-06-23.md`. v1 r2 archived at `hypothesis-v1-archived.md` (LOCKED 2026-06-23 `de22b68`; test-executed 2026-06-23 `a9423af`; HALT on dry-run §7.5 Gate 1). **Status: r2 LOCKED 2026-06-23.** Test execution is a separate post-lock session per the canonical arc.*
