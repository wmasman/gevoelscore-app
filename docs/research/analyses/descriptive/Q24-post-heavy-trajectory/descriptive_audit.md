...
**Status**: **LOCKED r4 2026-07-15** post small doc-hygiene addition (§10 item 8) elevating the `effective_exertion_min` +3d success DISCARD verdict from a technical dispositional note into a proper downstream-synthesis pickup flag (level-vs-change disagreement observation; not empirically null; narrative-elevatable at Stage S1/S2 when Q24 reaches those stages). Prior r3 content preserved byte-identically. Any downstream Stage H per-HA pre-registration draws on §5 branch verdicts + §6.3 sign-consistent survivor list + §6.4 sign-flipper disposition here; any downstream Stage S1/S2 synthesis reads §10 item 8 for the calibrated-partial-compensation observation.

**Prior status**: LOCKED r3 2026-07-15 (see §12 lock log) — Wave 2 cleanup wave (2 tasks): (a) §6.4 NEW subsection resolves the 2 sign-flipper cells surfaced in r2 §6.3 Class B via pre-episode-slope diagnostic — one **DISCARD**, one **RESCUABLE for Stage H with pre-committed direction = detrended**; (b) §7.4 integrity fix — the "~101 min" `sleep_duration_min` failure-pool AUC now anchored to the trajectory_summary.csv row (raw AUC = 101.005 min, CI [10.10, 192.28]).
...
**These sign-flippers are NOT dismissible artefacts AND NOT Stage H seeds.** They require diagnostic investigation before either dismissal or elevation. Raw and detrended pointing opposite directions typically means the raw signal and underlying drift are strongly co-varying in ways not yet understood — the detrend operation is removing a component that dominates the raw effect and flips it sign. `effective_exertion_min` at +3d success was already flagged in §4.1 as a "nuanced story" (definitional-pair signal); the sign-flip under detrending materially strengthens that caveat. `spo2_avg_sleep` at +3d failure is a novel diagnostic surface for Wave 2. **Wave 2 open item: diagnostic investigation of the 2 sign-flippers** (probe per-episode raw + pre-window overlays; check whether the flip is driven by a small subset of episodes or by a systematic drift-magnitude interaction; do NOT seed Stage H pre-regs on these cells until the flip mechanism is understood). **RESOLVED in r3 §6.4 below.**

### 6.4 Diagnostic sub-arm: 2 sign-flippers investigation (r3 Wave 2 close)

Investigates the mechanism of the 2 sign-flipper cells surfaced in §6.3 Class B. Executed by [`scripts/sign_flipper_diagnostic.py`](scripts/sign_flipper_diagnostic.py) (idempotent; `RANDOM_SEED = 20260715`; outputs to [`output/sign_flipper_diagnostic.csv`](output/sign_flipper_diagnostic.csv)). Method: for each anchor date in the trigger-arm pool AND the matched-ordinary comparator pool at the sign-flipper's (outcome × window × pool), fit the same `linear_detrend_on_pre` linear trend on the outcome column over the 30d pre-window `[D-30, D-1]` (identical to §7.11 detrend), extract the slope, and compare distributions. Also inspects the pre-window MEAN level and the trend-extrapolated baseline at k=1..w (what the detrend subtracts) to attribute the flip to a level-vs-change disagreement vs a slope disagreement.

**Reading key**: "level-vs-change disagreement" = trigger and comparator have systematically different pre-window ABSOLUTE LEVELS (raw AUC reads the level gap on top of the post-window observations; detrended AUC reads the change from each arm's own pre-window baseline). "Slope disagreement" = trigger arm's pre-window has a systematically different LINEAR TREND than comparator arm (drift-entanglement per §9.1 caveat 9; the flip is driven by the drift component being removed).

#### 6.4.1 `effective_exertion_min` +3d compensatory-success

- **What the flip is**: raw AUC = **+3.55** [+0.25, +7.17] (positive: trigger absolute exertion higher than comparator); detrended AUC = **−6.49** [−12.88, −0.47] (negative: trigger drops MORE from its own pre-window baseline than comparator does).

- **Pre-episode slope distribution**: trigger n=109, mean slope **−0.079/day** (median −0.017; IQR [−0.248, +0.213]); comparator n=312, mean slope **−0.036/day** (median +0.009; IQR [−0.162, +0.152]). Observed slope-diff (trigger − comparator) = **−0.043/day**; two-sided permutation p = **0.34** (B=10,000). **Slopes are NOT statistically distinguishable** between arms.

- **Pre-episode LEVEL distribution**: trigger mean pre-window daily value = **11.40 min/day** (n=109 pre-window means with ≥15 valid pre-points); comparator mean = **7.35 min/day**. Trend-extrapolated baseline at k=1: trigger **+10.09**, comparator **+6.70** (diff **+3.39**). At k=2 and k=3 the diff is ~+3.35 and ~+3.30 respectively (near-flat because slopes are near-zero).

- **Post-window absolute observations**: trigger raw per-day means at k=1/2/3 = **3.41 / 3.74 / 3.05** min/day; comparator raw = **2.38 / 2.14 / 2.12** min/day (raw delta +1.03/+1.59/+0.93). After subtracting the extrapolated pre-window baseline: trigger detrended per-day means = **−6.68 / −6.28 / −6.88** min/day; comparator detrended = **−4.30 / −4.51 / −4.54** min/day (detrended delta −2.38/−1.76/−2.34).

- **Mechanism identified**: **level-vs-change disagreement** on a definitional pair. Heavy episode-ends are BY DEFINITION days at the top of a run-up of exertion; the 30d pre-window ambient exertion level immediately before a heavy episode-end is **~55% higher** than the ambient exertion level immediately before a matched-ordinary day (11.40 vs 7.35 min/day). Post-episode-end, both arms drop to similar low absolute exertion levels (3.4 vs 2.4 min/day at k=1) — the trigger's post-window level is still higher than the comparator's (raw AUC +3.55) because the trigger arm's pre-window baseline was higher, but the trigger's DROP FROM ITS OWN BASELINE is larger (detrended AUC −6.49). Slopes are indistinguishable (p=0.34) — the flip is NOT a drift-entanglement mechanism per §9.1 caveat 9; it is a **level offset** mechanism at the trigger-arm construction stage.

- **Verdict**: **DISCARD — genuinely ambiguous.** The two readings answer structurally different questions and both are legitimate reads of "compensatory response":
    - Raw = "day-after-heavy absolute exertion is still higher than a matched-ordinary day's exertion, even after the compensatory drop" (~+3.55 AUC — the drop was not deep enough to bring absolute exertion below matched-ordinary; trigger arm ends the +3d window higher than comparator's low baseline).
    - Detrended = "day-after-heavy exertion drops MORE from the trigger arm's own recent baseline than a matched-ordinary day's exertion drops from its own recent baseline" (~−6.49 AUC — the compensatory response, measured as change-from-baseline, is larger on heavy episodes).
    Both are TRUE in the sense they describe the data faithfully; neither can be pre-committed as "the" primary reading without further theory. The `effective_exertion_min` composite axis is already flagged as a definitional-pair signal in §4.1 (composite catches a different signal than raw minute-counts); the sign-flip mechanism here compounds that ambiguity. **Not a Stage H seed.** The 3 raw activity channels (`total_steps`, `vigorous_min`, `active_min`) that DO show clean negative-direction raw AUC at +3d success remain the defensible activity-side compensatory signals; those are all detrend-erasing (§6.1) rather than sign-flipping, and the drift-entanglement reframe (§9.1 caveat 9) already applies. `effective_exertion_min` +3d success adds no independent signal beyond those three at Wave 1 sample sizes.

#### 6.4.2 `spo2_avg_sleep` +3d compensatory-failure

- **What the flip is**: raw AUC = **+1.01** [+0.01, +2.03] (positive: trigger absolute overnight SpO2 higher than comparator — direction-opposite the expected post-heavy hypoxia cost); detrended AUC = **−1.36** [−2.49, −0.19] (negative: trigger drops MORE from its own pre-window baseline than comparator does — direction-consistent with expected post-heavy cost).

- **Pre-episode slope distribution**: trigger n=16, mean slope **+0.0212/day** (median +0.0182; IQR [+0.0040, +0.0517]); comparator n=185, mean slope **−0.0053/day** (median −0.0061; IQR [−0.0221, +0.0123]). Observed slope-diff (trigger − comparator) = **+0.0265/day**; two-sided permutation p = **0.0003** (B=10,000). **Slopes are strongly statistically distinguishable** — the 16 failure-pool trigger episodes cluster on pre-windows with systematically POSITIVE SpO2 drift (~+0.021 percentage-points-per-day upward trend) while the matched-ordinary comparator pool has near-zero drift (~−0.005/day).

- **Pre-episode LEVEL distribution**: trigger mean pre-window daily value = **93.86%**; comparator mean = **93.51%** (diff +0.35 percentage points, small but non-zero). Trend-extrapolated baseline at k=1: trigger **+94.22**, comparator **+93.42** (diff **+0.79**); at k=2 diff **+0.82**; at k=3 diff **+0.85**. Diff grows across the post-window because trigger arm's positive slope pushes the extrapolated baseline upward faster than comparator's near-zero slope.

- **Post-window absolute observations**: trigger raw per-day means at k=1/2/3 = **94.14 / 93.77 / 93.87%**; comparator raw = **93.54 / 93.60 / 93.64%** (raw delta +0.61/+0.17/+0.23; small positive, consistent with the pre-window level offset). After subtracting the extrapolated baseline: trigger detrended = **−0.12 / −0.47 / −0.45**; comparator detrended = **+0.05 / +0.11 / +0.16** (detrended delta −0.17/−0.58/−0.61; consistently negative — heavy-failure episodes are BELOW their own recent baseline while comparators are slightly above).

- **Era clustering** of the 16 failure-pool episodes: 3 in 2022, 6 in 2023, 7 in 2024, **0 in 2025-2026**. This clusters in a corpus era where the SpO2 measurement baseline was recovering upward on the participant-level trend (positive drift). The 30d pre-window on each of these episodes captures that upward drift; the detrend removes it and reveals the post-episode SpO2 depression relative to the just-then-rising baseline. Direction (down = hypoxia cost) is consistent with the pre-committed direction on the compensatory-failure pool (heavy day followed by crash should show autonomic cost).

- **Mechanism identified**: **drift-entanglement per §9.1 caveat 9**. The raw finding is dominated by the positive pre-window drift on the trigger arm (trigger's absolute SpO2 baseline is on an upward era-trend); the detrend removes that drift and reveals the underlying event-triggered response (post-heavy-with-crash SpO2 drops from its just-then-rising baseline). This is the direct mirror of §6.2's "detrend-surfacing on failure pool" pattern: **envelope drift was creating an apparent absence-of-signal (raw AUC +1.01 in the wrong direction) that masks a real event-triggered cost signal (detrended AUC −1.36 in the pre-committed direction)**. The failure-pool era-clustering + rising baseline is precisely the era-stratified-comparator concern that §9 caveat 8 escalated per Q24 MD §10.8.

- **Verdict**: **RESCUABLE for Stage H with pre-committed direction = detrended (−1.36 AUC on the compensatory-failure pool, direction NEGATIVE = post-heavy-with-crash SpO2 drop relative to trigger-arm's own recent baseline)**. Pre-commit justification:
    - The compensatory-failure pool question is "does the post-heavy trajectory show an autonomic cost signal on days where the trajectory ends in a crash?"; the physiologically-meaningful comparison is CHANGE-FROM-BASELINE (does the participant's SpO2 drop below what their recent trend would predict?), NOT ABSOLUTE LEVEL (which is confounded by the era-recovery drift that has nothing to do with the specific heavy episode).
    - The raw reading (+1.01, direction-opposite expected) is a Q24 MD §7.7 sign-inversion finding on absolute level that IS DIRECTLY EXPLAINED by the drift-entanglement mechanism above; the finding is not that heavy-with-crash SpO2 is HIGHER than matched-ordinary (that would require a physiological mechanism), it is that heavy-with-crash EPISODES CLUSTER IN A HIGH-BASELINE ERA (that is an era-selection mechanism, not a physiological one).
    - Aligns with the §6.2 detrend-surfacing pattern (5 other cells on the compensatory-failure pool where envelope drift masks real signals visible only after detrending) — `spo2_avg_sleep` +3d failure joins that set on the same mechanism.
    - Stage H pre-reg pre-committed direction = NEGATIVE detrended AUC on the compensatory-failure pool; era-stratified comparator sensitivity arm per §9 caveat 8 escalation is REQUIRED at Stage H (armed here because this is a compensatory-failure autonomic-channel finding, which is precisely the escalation trigger). N=16 is small; Stage H sample-size discipline per Q24 MD §3.5 (>10 threshold satisfied) applies.
    - **Cross-reference**: this joins the 5 §6.3 sign-consistent survivors as a candidate Stage H seed at n=16 on the compensatory-failure pool at +3d — but with pre-committed direction = DETRENDED (not raw), which is the load-bearing distinction from the 5 sign-consistent survivors (whose raw AND detrended arms agree). Downstream Stage H drafting MUST honour this direction pre-commit and MUST report the raw arm as a documented sign-inversion attributable to era-clustering drift (§7.7 sign-inversion finding class with mechanistic explanation).

**Summary**: **1 DISCARD (`effective_exertion_min` +3d success), 1 RESCUABLE (`spo2_avg_sleep` +3d failure, pre-committed direction = detrended)**. The 2 sign-flippers had structurally different mechanisms — the first is a level-offset artefact on a definitional-pair composite (both readings legitimate; neither uniquely privileged; already-covered by 3 sibling activity channels); the second is a drift-entanglement mechanism in the direction §9.1 caveat 9 predicted (raw = upper bound corrupted by era-drift; detrended = lower bound revealing the underlying event-triggered signal in the pre-committed direction). Stage H seed set updates from "5 sign-consistent survivors" (r2 §6.3) to **"5 sign-consistent survivors + 1 detrended-only-defensible RESCUABLE sign-flipper"** with clear pre-commit boundaries on which direction each cell defends.
...
Both pools show positive `sleep_duration_min` AUC at +3d/+5d (extra sleep after heavy episode-end); the failure pool magnitude is slightly higher at +3d (**+101.0 min** CI [+10.1, +192.3], n_t=16, n_c=296; [`output/trajectory_summary.csv`](output/trajectory_summary.csv) row for `sleep_duration_min`/window=3/pool=compensatory_failure/arm=raw) vs the success-pool AUC (+74.15 min CI [+40.11, +107.45] per §4.2 sleep-architecture table, `sleep_duration_min`/window=3/pool=compensatory_success/arm=raw) but the direction and shape are similar. Both are trajectory-confound-suspect per §6 (`sleep_duration_min` +3d success is in the §6.1 detrend-erasing set; the failure-pool cell's detrended arm collapses to AUC +3.88 with a very wide CI [−113.47, +126.68] at n=16, so the failure-pool magnitude gap over the success pool does not survive detrending at Wave 1 sample sizes).
...
8. **`effective_exertion_min` +3d compensatory-success — level-vs-change disagreement observation (r4 flag; downstream-synthesis pickup candidate)** — the DISCARD verdict on this sign-flipper (§6.4) is a Stage-H-seeding call, NOT an empirical null. Both readings faithfully describe the data: raw AUC +3.55 (heavy-episode-ends still doing MORE activity than matched-ordinary in the +3d post-window on absolute-level terms) AND detrended AUC −6.49 (heavy-episode-ends dropped FURTHER from their own pre-episode baseline than matched-ordinary did from theirs). The pre-episode ambient level offset is ~+3.4 min/day (~55% higher trigger vs comparator per §6.4). This is a genuine level-vs-change ambiguity — both quantities describe distinct real phenomena on the corpus, and neither is a-priori the correct primary reading. **Downstream-synthesis pickup candidate**: when Q24 reaches Stage S1 (internal synthesis across HAs) or Stage S2 (external contextualisation vs literature), this observation is a narrative-elevatable finding in its own right — the participant maintains an elevated activity floor on days following heavy episodes (relative to matched-ordinary) even though they compensate relative to their own pre-episode baseline. This is neither compensation-visible-in-behaviour (in the naive sense) nor compensation-absent — it is a **calibrated-partial-compensation** pattern that both raw and detrended reads jointly characterise. **Do NOT re-open Stage D or Stage H for this cell**; do surface at Stage S1/S2 with the pointer to §6.4 for full mechanism. Also stands as a corpus-general observation: when a signal shows sign-flip between raw and detrended AND pre-episode slopes are indistinguishable but levels are different, level-vs-change disagreement is the correct framing, NOT drift-artefact.
...
| version | date | change |
|---|---|---|
| r4 (this) | 2026-07-15 | Small doc-hygiene addition per user disposition 2026-07-15: elevate `effective_exertion_min` +3d success DISCARD verdict (§6.4) into a proper downstream-synthesis pickup flag in §10 as item 8. Framing: the DISCARD is a Stage-H-seeding call, NOT an empirical null; both raw (+3.55) and detrended (−6.49) faithfully describe distinct real phenomena on the corpus; pre-episode ambient level offset ~+3.4 min/day (~55% higher trigger vs comparator per §6.4). Reframes the observation as **calibrated-partial-compensation** (elevated activity floor + own-baseline drop) that both readings jointly characterise. Explicit instruction to downstream-synthesis orchestrator: do NOT re-open Stage D or Stage H for this cell; DO surface at Stage S1 (internal synthesis) or Stage S2 (external contextualisation) with pointer to §6.4 for full mechanism. Also stands as a corpus-general observation: when a signal shows sign-flip between raw and detrended AND pre-episode slopes are indistinguishable but levels differ, level-vs-change disagreement is the correct framing (NOT drift-artefact). Ensures the observation is not buried in a technical §6.4 diagnostic subsection when future orchestrator picks up Q24 downstream synthesis work. STATUS transitions: LOCKED r3 → LOCKED r4. No other content changed. |
| r3 | 2026-07-15 | Wave 2 cleanup wave — 2 tasks close outstanding Wave 1 open items surfaced during r2 review. (a) NEW **§6.4** subsection "Diagnostic sub-arm: 2 sign-flippers investigation" resolves the 2 sign-flipper cells (`effective_exertion_min` +3d success + `spo2_avg_sleep` +3d failure) surfaced in r2 §6.3 Class B; new script `scripts/sign_flipper_diagnostic.py` (`RANDOM_SEED = 20260715`; permutation B=10,000 for slope-diff test) writes `output/sign_flipper_diagnostic.csv` (gitignored; regenerable). Verdicts: **`effective_exertion_min` +3d success = DISCARD** (level-vs-change disagreement on a definitional-pair composite; slopes indistinguishable at p=0.34; the 3 sibling activity channels already carry the compensatory-signal load); **`spo2_avg_sleep` +3d failure = RESCUABLE for Stage H with pre-committed direction = detrended AUC = −1.36** (drift-entanglement mechanism confirmed at pre-episode slope-diff p=0.0003; failure-pool era-clustering 2022-2024, zero 2025-2026; raw sign-inversion is an era-selection artefact, not a physiological finding). Stage H seed set updated from "5 sign-consistent survivors" (r2 §6.3) to "5 sign-consistent survivors + 1 detrended-only-defensible RESCUABLE sign-flipper". Preamble-line in §6.3 Class B updated with forward-reference to §6.4. (b) **§7.4 integrity fix**: the "~101 min" `sleep_duration_min` failure-pool figure now anchored to trajectory_summary.csv row (raw AUC = +101.0 min CI [+10.1, +192.3], n_t=16, n_c=296) with the mirror success-pool figure explicitly citing §4.2, and the detrended-arm collapse (AUC +3.88 CI [−113.47, +126.68]) noted so the failure-vs-success magnitude gap is correctly framed as raw-only. STATUS transitions: LOCKED r2 → LOCKED r3. Wave 2 open items remaining after this cleanup: Q24 sub-part 3 phase-stratification; intensity-stratified + inclusive-overlap arms; Stage H pre-reg drafting (now on 5 + 1 seed set); era-stratified comparator sensitivity arm required at Stage H per §9 caveat 8 for the RESCUABLE sign-flipper (armed per Q24 MD §10.8 escalation rule because the RESCUABLE cell is an autonomic channel on the compensatory-failure pool). |
| r2 | 2026-07-15 | Fresh-session `/research-review` verdict **PASS with caveats** at [`../../../reviews/Q24-post-heavy-trajectory-2026-07-15.md`](../../../reviews/Q24-post-heavy-trajectory-2026-07-15.md) (139 lines). Six substantive absorbs applied (patches 1-6): (1) §6.3 arithmetic corrected from "7 survivors" to "5 sign-consistent Stage-H-candidate survivors + 2 sign-flip diagnostic cells requiring Wave 2 investigation" (L4 fire #1; load-bearing for Stage H seeding). (2) NEW §9.1 caveat 9 drift-entanglement reframe (user-endorsed framing 2026-07-15): raw = upper bound; detrended = lower bound; truth between; 36 drift-entangled cells are candidates for Q24 sub-part 3 disambiguation, NOT dismissible artefacts. Provides explicit Wave 1 → sub-part 3 motivating link. (3) §4-§8 `n_c` column corrections after CSV verification (L4 fire #2; downstream statistical-power reasoning unaffected; only tabulated n_c figures wrong; ~2× undercount confirmed and fixed against CSV `n_comparator_days` column). (4) §5 branch verdicts cross-annotated with §6 detrend fragility via preamble + per-cell `[frag]` / `[surface]` footnotes (L3.5; prevents over-weighting the Radin 2024 delayed-HR-decay reading given the underlying raw trajectories' §6.1 fragility status). (5) §9 caveat 8 era-stratified comparator sensitivity escalated from generic Wave 2 candidate to triggered escalation per Q24 MD §10.8 rule (L2.2; 44/118 fragility crosses the threshold). (6) §12 lock log + status header update. STATUS transitions: LOCKED r1 → LOCKED r2. Wave 2 open items logged: diagnostic investigation of 2 sign-flippers (`effective_exertion_min` +3d success + `spo2_avg_sleep` +3d failure); Q24 sub-part 3 phase-stratification (now with explicit motivating link per §9.1); intensity-stratified + inclusive-overlap arms; Stage H pre-reg drafting on the 5 sign-consistent survivors (all on compensatory-failure sub-arm at +3d/+5d, activity + sleep + subjective channels). |
