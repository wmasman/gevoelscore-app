...
**Status**: **LOCKED r3 2026-07-15** post Wave 2 cleanup wave (2 tasks; see §12 lock log): (a) §6.4 NEW subsection resolves the 2 sign-flipper cells surfaced in r2 §6.3 Class B via pre-episode-slope diagnostic — one **DISCARD**, one **RESCUABLE for Stage H with pre-committed direction = detrended**; (b) §7.4 integrity fix — the "~101 min" `sleep_duration_min` failure-pool AUC now anchored to the trajectory_summary.csv row (raw AUC = 101.005 min, CI [10.10, 192.28]). Prior r2 producer-mode audit content preserved byte-identically elsewhere. Any downstream Stage H per-HA pre-registration draws on §5 branch verdicts + §6.3 sign-consistent survivor list + §6.4 sign-flipper disposition here.
