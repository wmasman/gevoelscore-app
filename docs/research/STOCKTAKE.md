...
- **[HA-C4 v2](analyses/hypotheses/HA-C4/hypothesis.md)** — **LOCKED 2026-06-18 r2** (`b0f38a7`) by user acceptance. Full v2 lock arc completed today: draft (`59fae4b` + `73cb2d4`) → fresh-session `/research-review` audit (verdict **PASS-with-caveats**; report at [`reviews/HA-C4-v2-2026-06-18.md`](reviews/HA-C4-v2-2026-06-18.md); 1 L2.5 substantive + 1 L4.5 minor + 4 side observations) → r2 absorbed 4 mechanical wording closures (`dee5885`) → §3.6 re-audit COMPRESSED per criteria (r2 changes mechanical; no architectural change) → lock + register pointer + README update (`b0f38a7`). **L2.5 disposition**: user accepted at lock time that v1 r2 "priced-in" shared-context drafting disposition carries forward to v2 (structural inheritance from v1's pre-dry-run bands materially weakens calibration-bias link). **Test handoff ready** at [`session-c4-v2-test-handoff-2026-06-18.md`](file:///C:/Users/Gebruiker/.claude/plans/session-c4-v2-test-handoff-2026-06-18.md). 3-channel Wiggers C4 confirmatory triad (Tier 1). Historical lineage: v1 LOCKED 2026-06-17 → dry-run HALTED 2026-06-17 on Ch3 validate n=25 < 30 → archived at `hypothesis-v1-archived.md`.
...
1. **Bout-level methodology MDs fresh-session audit** — `bout_level_recovery_dynamics.md` + `bout_level_dose_response_calibration.md` drafted today r1 (2026-06-19); next step is `/research-methodology-review` on each MD in fresh sessions per [CONVENTIONS §2.2](CONVENTIONS.md#22-methodology-md-before-locking-a-major-choice). Single audit report covering BOTH MDs lands at `reviews/bout_level_recovery_dynamics-YYYY-MM-DD.md` (no `methodology-` prefix; single-report-for-paired-MDs per the audit-handoff convention). After PASS / PASS-with-caveats, r2-absorb + lock. After both MDs lock: pipeline construction session (`pipeline/02_features/extract_stress_bouts.py` + per-bout dataset); then HA11-bout-redo pre-reg (framework-validity reproduction check, restricted to unmedicated stratum × train era × calm days). HALT if HA11 v1 reproduction fails — substantive HA-C4c cascade halts until methodology r2.
