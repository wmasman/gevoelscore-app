...
| 2026-06-05 | H03 | HA-test | NULL | Sleep efficiency drop before crashes — refuted decisively both eras (0% of episodes crossed −5 pp threshold). Sleep efficiency is flat for this user. (NB: the "sharper metrics" follow-up flagged here — deep-sleep %, REM %, fragmentation — was superseded, never formalised; the H03b label was reused for the per-minute BB recharge test in the row below, so a sleep-architecture retest would be a NEW HA, not H03b.) | [H03-sleep-efficiency/result.md](analyses/hypotheses/H03-sleep-efficiency/result.md) |
...
| 2026-06-14 | RESP-SSRI | informal multi-channel | DESCRIPTIVE-FAIL | `respiration_avg_sleep` × citalopram dose-response — no signal in afbouw (β=−0.002, p=0.57) or buildup post-CPAP (β=−0.011, p=0.86) per the v3 multi-channel sweep. Wichniak 2017 sleep-architecture prior NOT supported for sleep respiration rate on this corpus at the 8-30mg dose range. Informative-by-rejection: rules out a "spring moves everything" artefact for the v3 multi-channel CONFIRMED set. | [citalopram_dose_response §5.6.1](methodology/citalopram_dose_response_stress_mean_sleep.md#561-per-channel-three-pronged-read) + [multi_channel_check.py](analyses/garmin_exploration/intervention_effects/multi_channel_check.py) |
| 2026-06-14 | INT-NARROW | informal cross-method | SUPERSEDED | Pre-v3 Session C reading "only `stress_mean_sleep` × 2026-03-20 candidate confirmed" (parent MD §8.3 + pre-v3 garmin_pacing_practice §7.4 "BB thresholds survive intact") superseded by v3 dose-response multi-channel work showing 3 channels (`stress_mean_sleep`, `all_day_stress_avg`, `bb_lowest`) confirmed across both Citalopram phases. The pre-v3 narrowing understated the breadth of the dose-response. | [citalopram_dose_response §5.5-§5.6](methodology/citalopram_dose_response_stress_mean_sleep.md#56-v3-amendment--multi-channel-confirmation-added-2026-06-14) |
| 2026-06-15 | HA-P7 | HA-test (Personal-register P7) | NULL | Recent-crash-density predicts elevated crash risk — refuted on all three Sec 5.1 criteria at the locked pooled-LC × W=14 headline (OR 1.130 [0.875, 1.266] at E[L]=7 contains 1; monotonicity violated under relative-50% gate; 3-of-3 W-window CIs contain 1; block-permutation p=0.17). Train (OR 1.121) and validate (OR 1.015) both NOT-SUPPORTED. §4.5.4 covariate sensitivity (adding `gevoelscore_lagged_mean_14d`) attenuates β_crash_count_14d to 0.941: the marginal signal reads as a recent-low-gevoelscore proxy, not an independent recovery-debt mechanism. E[L]* factor-of-2 flag (E[L]*=11.97 vs locked E[L]=7) reviewed as structural rolling-sum-predictor autocorrelation (~13/14 day-overlap); sensitivity addendum confirmed verdict robust across E[L] ∈ {7, 12, 14}. First Personal-register inferential test in the project; surfaced the rolling-sum predictor structural-autocorrelation pattern canonicalised in [`hypothesis_lock_process.md` §4.6](methodology/hypothesis_lock_process.md#46-result-time--rolling-sum-predictor-structural-autocorrelation-factor-of-2-flag). | [HA-P7/result.md](analyses/hypotheses/HA-P7/result.md) + [verdict-review.md](analyses/hypotheses/HA-P7/verdict-review.md) |
