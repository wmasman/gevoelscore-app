...
**Statistical credibility.** Two pre-registered tests SUPPORTED for train-era crashes on independent operationalisations: H02b at 3-day lead-up (+29.9 pp discrimination, 71% of train crashes) and H02d at 5-day lead-up with bridge-arm sentinel handling (+31.8 pp discrimination — the strongest train-era single-channel signal of the whole project). H02d's bridge train discrimination scaled monotonically with window (3d → 4d → 5d = +29.9 → +27.6 → +31.8 pp), corroborating the user's 4-5 day empirical PEM lag. **Era-shift caveat**: across five stress-channel tests (H02 daily-avg, H02b 3d, H02d × 4 arms), validate-era crashes consistently refute — the stress-spike precursor is a pre-cliff phenomenon that did not survive the stabilisation transition. For 2022-23 retrospective surfacing, the indicator is well-grounded. For 2024+ live use, the daily stress spike should not be promised as a precursor: only ~33% of validate-era crashes have a clear lead-up spike pattern, against a ~40% null-window rate. Worth surfacing retrospectively, not predictively.
...
- **A composite "crash risk" percentage**. We do not have a calibrated multi-day predictor. Any single percentage would be either dishonestly precise or trivially the base rate. Refuse to build until a pre-registered HA01c or successor is SUPPORTED on truly held-out data.
...
- **Era split**. The two SUPPORTED indicators (H02b, HA01b) operate at different lags (3-day vs 4-day) in different eras of the participant's stabilisation trajectory. This is a feature, not a bug — different crash mechanisms are likely dominant at different stages — but it means any indicator is more credible for some crash modes than others.
