...
**Status:** Designed — brainstorm resolved 2026-06-02. Step file pending — wait for `verloop-and-episodes` schema to land first (this feature depends on `episodes` table + `tags.parent_episode_id` existing).
...
- **Why:** A score line alone is one signal. Overlaying the things that were happening turns the timeline from "diary of feelings" into "diagnostic surface" — patterns the user can act on. This is the second-half deliverable of [ADR 0006](../../decisions/0006-three-surface-architecture.md); Verloop is "manage the episodes", Tijdlijn-overlay is "see them in context".
...
1. **Line chart — episode bands**:
   ...
   - Rectangle is tappable: opens the episode's detail screen in Verloop (cross-tab nav).
...
- **Editing an episode from the timeline**. Tapping a band navigates to Verloop's episode detail; doesn't edit in place.
...
- **Episode → tap → Verloop nav**: needs a route-level coordination so the Verloop tab opens to the right episode's detail. Possibly query-param `?episode=<id>`. Confirm shape in `/plan-feature`.
