# Wrapper for scripts/episodes-smoke.mjs that sources WILLEM_EMAIL,
# WILLEM_PASSWORD, and DIRECTUS_TOKEN from the project's .env.local
...
if (-not $env:DIRECTUS_TOKEN) {
  ...
  Write-Host 'Needed for cleanup hard-DELETE of the smoke test row.'
  exit 2
}

Push-Location (Resolve-Path (Join-Path $PSScriptRoot '..'))
try {
  node scripts/episodes-smoke.mjs
  exit $LASTEXITCODE
} finally {
  Pop-Location
}
