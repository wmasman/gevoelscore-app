# Wrapper for scripts/tag-management-smoke.mjs that sources WILLEM_EMAIL,
# WILLEM_PASSWORD, and DIRECTUS_TOKEN from .env.local (gitignored).
#
# Usage:
...
try {
  node scripts/tag-management-smoke.mjs
  exit $LASTEXITCODE
} finally {
  Pop-Location
}
